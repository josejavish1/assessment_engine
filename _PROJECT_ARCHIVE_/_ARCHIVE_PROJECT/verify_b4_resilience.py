...
def test_resilience():
    ...
    try:
        ...
        print(f"   - ¿Tiene metadatos?: {'Sí' if payload.generation_metadata else 'No'}")
        
        # Verificar que el objeto existe aunque falten campos
        if not hasattr(payload, 'executive_summary') or payload.executive_summary is None:
            print("   - Confirmado: El campo 'executive_summary' falta pero el objeto es usable.")
