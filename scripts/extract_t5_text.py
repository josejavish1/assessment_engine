...
def audit_t5():
    payload_path = Path("working/redeia_v3/T5/blueprint_t5_payload.json")
    ...
    print("=====================================================================")
    print("🔍 AUDITORÍA ARQUITECTÓNICA - TORRE 5 (RESILIENCE & CONTINUITY)")
    print("=====================================================================\n")
    ...
    for idx, pillar in enumerate(pillars, 1):
        ...
        for proj in projects:
            print(f"         • {proj.get('name')} (Sizing: {proj.get('sizing')} | Duración: {proj.get('duration')})")
    ...
    # 5. Diplomatic tone violations check
    prohibited_words = ["desastre", "caos", "inaceptable", "desastroso", "insuficiente"]
    text_str = json.dumps(data).lower()
    found_violations = [w for w in prohibited_words if w in text_str]
    print(f"\n🛡️ COMPROBACIÓN DE PROTOCOLO DIPLOMÁTICO TIER 1:")
    if found_violations:
        print(f"   ❌ ALERTA: Se han encontrado términos no-diplomáticos en el payload: {found_violations}")
    else:
        print("   ✅ SUCCESS: 100% de sobriedad ejecutiva y tono diplomático cumplido.")
