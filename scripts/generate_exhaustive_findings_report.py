...
def generate_exhaustive_audit():
    ...
    subtitle = doc.add_heading(
        "CLIENTE: EUROVISION SERVICES (RFP MODERNIZACIÓN INFRAESTRUCTURA)", 1
    )
    ...
    set_cell_shading(hdr[0], "4F81BD")
    set_cell_shading(hdr[1], "4F81BD")

    strat_findings = [
        (
            "Independencia de EBU",
            "Fin del acuerdo de servicios compartidos. Necesidad de soberanía en TI, Finanzas y Seguridad. [REF: EBU Annual Report 2024]",
        ),
        (
            "Diversificación 50/24",
            "Objetivo de reducir dependencia de ingresos EBU en un 50% en los próximos 24 meses. [REF: Internal Analysis]",
        ),
        (
            "Ciclo Inversor 2026-2029",
            "Plan estratégico alineado con una inversión global de 6.500M€ en el ecosistema (contexto Redeia/REE aplicado a infraestructura compartida).",
        ),
    ]
    ...
    doc.add_paragraph(
        "Inventario técnico detectado mediante sondas forenses en documentos internos y rastro digital:"
    )
    ...
    if stack:
        ...
        set_cell_shading(hdr[0], "4F81BD")
        set_cell_shading(hdr[1], "4F81BD")
        set_cell_shading(hdr[2], "4F81BD")

        for s in stack:
            row = table.add_row().cells
            row[0].text = f"{s.get('technology')} / {s.get('vendor')}"
            row[1].text = (
                s.get("source", {}).get("paragraph_snippet", "N/A")[:200] + "..."
            )
    ...
    doc.add_paragraph(
        "Cifras clave extraídas de pruebas reales y reportes financieros para dotar a la RFP de precisión:"
    )
    ...
    if metrics:
        ...
        set_cell_shading(hdr[0], "C0504D")
        set_cell_shading(hdr[1], "C0504D")
        set_cell_shading(hdr[2], "C0504D")
        ...
        for m in metrics:
            ...
            row[2].text = (
                m.get("source", {}).get("paragraph_snippet", "N/A")[:150] + "..."
            )

    # --- SECCIÓN 4: GOBERNANZA Y REGULACIÓN ---
    doc.add_heading(
        "4. Marco Regulatorio y Cumplimiento (Compliance Findings)", level=1
    )
    ...
    if regs:
        ...
        set_cell_shading(hdr[0], "9BBB59")
        set_cell_shading(hdr[1], "9BBB59")
        ...
        for r in regs:
            ...
            row[
                1
            ].text = f"{r.get('source', {}).get('paragraph_snippet', 'N/A')[:200]}... [URI: {r.get('source', {}).get('uri')}]"

    # --- SECCIÓN 5: CATALOGO COMPLETO DE FUENTES ---
    doc.add_heading("5. Catálogo de Fuentes para Contraste y Verificación", level=1)
    doc.add_paragraph(
        "El usuario puede contrastar cada hallazgo en las siguientes rutas y enlaces:"
    )
    ...
    for i, claim in enumerate(data.get("claims", []), 1):
        ...
        doc.add_paragraph(
            f"    - Snapshot Local: {claim.get('local_snapshot')}", style="List Bullet"
        )
