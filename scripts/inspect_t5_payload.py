...
def run_deep_t5_audit():
    ...
    print(
        f"   ├─ Cliente: {meta.get('client_name')} (ID: {meta.get('client_slug', 'N/A')})"
    )
    ...
    print(
        f'   ├─ Cost of Inaction (Coste de Inacción): "{exec_snap.get("cost_of_inaction")}"'
    )
    print(
        f"   └─ Complejidad de la Transformación: {exec_snap.get('transformation_complexity')}\n"
    )
    ...
    for p_idx, pilar in enumerate(pillars, 1):
        ...
        grounded_evidences = sum(
            1
            for f in pilar.get("health_check_asis", [])
            if f.get("literal_evidence") or f.get("fragment_id")
        )
        ...
        print(
            f"      ├─ Trazabilidad Evidencias: {grounded_evidences}/{total_evidences} con citas RAG reales."
        )
        print(f'      ├─ Visión To-Be: "{vision[:220]}..."')
        print(f"      └─ Proyectos Técnicos ({len(projs)}):")
        for proj in projs:
            print(
                f"         • Proyecto: {proj.get('name')} | Duración: {proj.get('duration')} | Sizing: {proj.get('sizing')}"
            )
            print(
                f'           └─ Objetivo Técnico: "{proj.get("tech_objective")[:180]}..."'
            )
        print()
    ...
    if p1 and p3:
        ...
        print(
            "   ├─ Análisis de Sequenciamiento (P1 Continuidad vs P3 Disaster Recovery):"
        )
        ...
        if h1_p3 and not h1_p1:
            print(
                "      • ⚠️ INCOHERENCIA ENCONTRADA: Se está proponiendo automatizar la recuperación (P3) en H1, pero la definición estratégica de RTO/RPO por negocio (P1) no se completa hasta H2. ¡Riesgo de sobre-ingeniería o diseño a ciegas!"
            )
            has_inconsistencies = True
        else:
            print(
                "      • ✅ Coherencia en Sequenciamiento: Los RTO/RPO estratégicos (H1) preceden o coinciden con la orquestación técnica de DR (H2/H3)."
            )
    ...
    if p2 and p4:
        ...
        if "vault" in p2_vision and "cyber" in p2_vision:
            print(
                "      • ⚠️ INCOHERENCIA DE CONCEPTOS: El Pilar P2 (Backup Ordinario) menciona capacidades de Cyber Vault que pertenecen exclusivamente a P4. Esto diluye el aislamiento de seguridad."
            )
        else:
            print(
                "      • ✅ Coherencia de Conceptos: P2 se enfoca en consolidación operativa y ciclo de vida ordinario en AWS Backup, mientras que P4 se enfoca estrictamente en aislamiento inmutable (Cyber Vault) en una cuenta AWS aislada."
            )
