...
def clean_todo_title(raw_title: str) -> str:
    ...
    for p in reversed(parts):
        if p and not p.lower().startswith('type:ignore') and not p.lower().startswith('type: ignore'):
            todo_part = p
            break
            
    cleaned = re.sub(r'^TODO\s*(?:\([^)]*\))?\s*:?\s*', '', todo_part, flags=re.IGNORECASE)
...
def create_github_issue(raw_desc: str, orig_comment: str, file_path: Path, project_root: Path) -> Optional[str]:
    """
    ...
    try:
        ...
        if match:
            ...
            logger.info(f"✓ Created GitHub Issue #{issue_num} for TODO: '{clean_title}'")
            return f"#{issue_num}"
        ...
        logger.warning("Connection timeout to GitHub API. Falling back to offline tracking.")
    except subprocess.CalledProcessError as e:
        logger.debug(f"GitHub CLI issue creation failed (unauthenticated or no remote origin): {e.stderr}")
...
class TODOTrackingTransformer(cst.CSTTransformer):
    ...
    def leave_Comment(self, original_node: cst.Comment, updated_node: cst.Comment) -> cst.Comment:
        ...
        if TODO_PATTERN.search(orig_val) and not TRACKED_TODO_PATTERN.search(orig_val):
            ...
            issue_id = create_github_issue(todo_desc, orig_val, self.file_path, self.project_root)
            ...
            if not issue_id:
                ...
                logger.info(f"⚠️ Offline/Unauthenticated: Assigned sequential placeholder {issue_id} to TODO: '{todo_desc}'")
...
def main() -> None:
    ...
    if len(sys.argv) > 1:
        py_files = [Path(arg).resolve() for arg in sys.argv[1:] if Path(arg).suffix == ".py"]
    else:
    ...
    for f in py_files:
        if process_file(f, current_dir):
            modified_count += 1
            print(f"✓ Automatically tracked pending TODOs in: {f.relative_to(current_dir)}")
