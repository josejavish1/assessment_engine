...
def compare_intel():
    ...
    print(f"   - Viejo Sector (Industry): {old_data.get('profile', {}).get('industry')}")
    print(f"   - Nuevo Sector (Industry): {new_data.get('profile', {}).get('industry')}")
    print()
    ...
    all_towers = sorted(list(set(list(old_overrides.keys()) + list(new_overrides.keys()))), key=lambda x: int(x[1:]) if x[1:].isdigit() else x)
    
    print(f"   {'Torre':<6} | {'Target Viejo':<13} | {'Target Nuevo':<13} | {'Cambio en Target / Justificación'}")
    print(f"   {'-'*6} + {'-'*13} + {'-'*13} + {'-'*45}")
    for tower in all_towers:
        ...
        print(f"   {tower:<6} | {str(old_val):<13} | {str(new_val):<13} | {new_just}...")
    print()
    
    # 3. Comparar el Footprint y Contexto Tecnológico (Narrativa)
    old_summary = old_data.get("technology_context", {}).get("footprint_summary", {}).get("summary", "N/A")
    new_summary = new_data.get("technology_context", {}).get("footprint_summary", {}).get("summary", "N/A")
    ...
    print(f"   - Encontrada en Viejo: {old_contaminations if old_contaminations else '¡Ninguna!'}")
    print(f"   - Encontrada en Nuevo: {new_contaminations if new_contaminations else '¡Ninguna!'}")
    print("\n" + "="*69 + "\n")
