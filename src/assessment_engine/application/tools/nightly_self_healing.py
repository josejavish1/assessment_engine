...
FENCED_BLOCK_RE = re.compile(r"```(?:bash|sh|shell|console|python)?\n(.*?)\n```", re.DOTALL)
...
def update_front_matter_status_to_needs_review(path: Path) -> None:
    ...
    print(f"  [DECAY] Modificado front matter del archivo físico a 'Needs Review': {path}")


def update_map_status_to_needs_review(map_path: Path, doc_path: str) -> None:
    text = map_path.read_text(encoding="utf-8")
    escaped_doc_path = re.escape(doc_path)
    pattern = re.compile(rf"(- path:\s+{escaped_doc_path}\b.*?)(?=\n- path:|\Z)", re.DOTALL)
    ...
    if match:
        ...
        print(f"  [DECAY] Modificado status a 'Needs Review' en documentation-map.yaml para: {doc_path}")
...
def add_source_of_truth_to_front_matter(path: Path, new_srcs: list[str]) -> None:
    ...
    if changed:
        ...
        parts[1] = new_fm_text
        path.write_text("---\n".join(parts), encoding="utf-8")
        print(f"  [SOT REPARADO] Se añadieron dependencias en el front matter de {path}: {new_srcs}")


def add_source_of_truth_to_map(map_path: Path, doc_path: str, new_srcs: list[str]) -> None:
    text = map_path.read_text(encoding="utf-8")
    escaped_doc_path = re.escape(doc_path)
    pattern = re.compile(rf"(- path:\s+{escaped_doc_path}\b.*?)(?=\n- path:|\Z)", re.DOTALL)
    ...
    if match:
        ...
        if changed:
            ...
            new_block_content = yaml.safe_dump(data, sort_keys=False, default_flow_style=False)
            indented_lines = []
            ...
            text = text.replace(block, new_block)
            map_path.write_text(text, encoding="utf-8")
            print(f"  [SOT REPARADO] Se añadieron dependencias en el documentation-map.yaml para: {doc_path}")
...
def repair_stale_command_in_file(file_path: Path, old_cmd: str, repo_root: Path) -> str | None:
...
def run_command_safely(cmd: str, repo_root: Path) -> tuple[int, str]:
    ...
    cmd_run = cmd_run.replace("/ruta/al/contexto.docx", "templates/golden_paths/README.md")
    cmd_run = cmd_run.replace("/ruta/a/respuestas.txt", "templates/golden_paths/README.md")
...
def run_runnable_docs_validation(repo_root: Path, map_path: Path, doc_path: str, file_path: Path) -> bool:
    ...
    for block in blocks:
        ...
        for line in lines:
            ...
            if (line_stripped.startswith("./.venv/bin/python") or 
                line_stripped.startswith("python") or 
                line_stripped.startswith("pytest") or 
                line_stripped.startswith("PYTHONPATH=src")):
                
                code, output = run_command_safely(line_stripped, repo_root)
                last_cmd_output = output
                if code != 0:
                    print(f"  [ERROR] Falló comando en {doc_path}. Intentando autocuración activa de ruta...")
                    new_cmd = repair_stale_command_in_file(file_path, line_stripped, repo_root)
                    if new_cmd:
    ...
    print(f"  [ÉXITO] Todos los snippets ejecutables y aserciones de {doc_path} pasaron correctamente.")
    return True


def run_ai_semantic_auditor_validation(repo_root: Path, map_path: Path, doc_path: str, file_path: Path, source_of_truth: list[str], api_key: str) -> bool:
    print(f"=== [EVOLUCIÓN 2: AI SEMANTIC AUDITOR] Evaluando {doc_path} contra schemas ===")
    
    schemas_content_list = []
    for s_path in source_of_truth:
        resolved_s = (repo_root / s_path).resolve()
        if resolved_s.exists() and resolved_s.is_file() and s_path.endswith((".py", ".json")):
...
"""
    ...
    if res_json.get("status") == "repaired":
        ...
        reason = res_json.get("reason", "Unknown discrepancy")
        print(f"  [ALARMA] Auditoría semántica falló para {doc_path} sin opción de reparación.")
...
def run_automated_stale_decay_validation(map_path: Path, doc_path: str, file_path: Path, last_verified: date | None) -> bool:
    ...
    print(f"  [ÉXITO] El documento se encuentra dentro del rango de frescura ({age_days}/120 días).")
    return True


def discover_and_register_untracked_md(repo_root: Path, map_path: Path, tracked_paths: set[str]) -> None:
...
"""
            ...
            print(f"    [REPARADO] Escrita cabecera por defecto en archivo físico: {rel_posix}")
        ...
        entry_yaml = {
            ...
            "last_verified_against": fm.get("last_verified_against", date.today().isoformat())
        }
        
        entry_block_content = yaml.safe_dump(entry_yaml, sort_keys=False, default_flow_style=False)
        indented_lines = []
    ...
    print(f"  [ÉXITO] Se agregaron {len(untracked_found)} nuevos registros de Markdown al mapa de documentación.")


def discover_dynamic_source_of_truth_dependencies(repo_root: Path, map_path: Path, doc_path: str, file_path: Path) -> None:
    print(f"=== [BLINDAJE 2] Escaneando dependencias de importación para {doc_path} ===")
    ...
    for rel_s in referenced_scripts:
        ...
        try:
            ...
            import_matches = re.findall(r"\bfrom\s+(assessment_engine\.[a-zA-Z0-9_\.]+)\s+import\b", content)
    ...
    if missing_sots:
        print(f"  [SOT] Descubiertas {len(missing_sots)} dependencias transitivas faltantes para {doc_path}!")
        # Add to Markdown front matter
        add_source_of_truth_to_front_matter(file_path, [md_p for _, md_p in missing_sots])
        # Add to documentation-map.yaml
        add_source_of_truth_to_map(map_path, doc_path, [map_p for map_p, _ in missing_sots])


def main() -> int:
    parser = argparse.ArgumentParser(description="Nightly Active Self-Healing and Documentation Decay Engine")
    parser.add_argument("--repo-root", default=".", help="Ruta de la raíz del repositorio")
    parser.add_argument("--documentation-map", default="docs/documentation-map.yaml", help="Ruta del mapa de documentación")
    ...
    api_key = os.environ.get("GEMINI_API_KEY") or os.environ.get("VERTEX_AI_API_KEY") or os.environ.get("GOOGLE_API_KEY")
    ...
    for entry in entries:
        ...
        if isinstance(last_verified_val, date):
            ...
            try:
                last_verified_date = datetime.strptime(last_verified_val, "%Y-%m-%d").date()
            except ValueError:
                pass

        # 1. Automated Stale Decay
        if not run_automated_stale_decay_validation(map_path, doc_path, file_path, last_verified_date):
            failures += 1
            continue
        ...
        discover_dynamic_source_of_truth_dependencies(repo_root, map_path, doc_path, file_path)
        ...
        if api_key and source_of_truth:
            if not run_ai_semantic_auditor_validation(repo_root, map_path, doc_path, file_path, source_of_truth, api_key):
                failures += 1
                continue
        elif not api_key:
            print("  [INFO] No se detectó ninguna API key de Gemini. Omitiendo auditoría semántica con IA.")
