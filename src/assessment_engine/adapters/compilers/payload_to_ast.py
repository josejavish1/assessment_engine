...
class PayloadToASTBridge:
    ...
    def __init__(
        self,
        brand_profile_path: str = "engine_config/brand_profile.json",
        locales_path: str = "engine_config/locales.json",
    ):
    ...
    def convert(
        ...
        global_score = (
            blueprint_data.get("executive_summary", {})
            .get("global_score", "3.0")
            .split()[0]
        )
        global_band = blueprint_data.get("executive_summary", {}).get(
            "global_band", "Definido"
        )
        global_reading = (
            blueprint_data.get("pillar_score_profile", {})
            .get("pillars", [{}])[0]
            .get("executive_reading", "Gobernanza estándar.")
        )
        ...
        formatted_global_score = (
            f"{gs_val:.2f}".replace(".", ",") if meta_lang == "es" else f"{gs_val:.2f}"
        )
        display_score_str = (
        ...
        nodes.append(
            HeadingNode(
                text=f"Anexo Técnico: {tower_name}",
                level=2,
        ...
        nodes.append(
            ParagraphNode(text=f"Proyecto:\n{project_name}", bold=True, space_after=100)
        )

        # Date and classification
        nodes.append(
            ParagraphNode(
                text=f"{date_str} | Versión {version} | {self.classification}",
                italic=True,
                text_color_rgb=[150, 150, 150],
            )
        )
        ...
        nodes.append(
            HeadingNode(
                text=vocab.get("toc_title", "Contenido"),
                level=4,
                primary_color_rgb=self.primary_color_rgb,
            )
        )
        nodes.append(
            ParagraphNode(
                text="[Índice de Contenidos Generado Dinámicamente al Abrir el Documento]",
                italic=True,
        ...
        if resumen_path.exists():
            ...
            nodes.append(
                HeadingNode(
                    text="1. Resumen Ejecutivo y Contexto de Negocio",
                    level=1,
                    primary_color_rgb=self.primary_color_rgb,
                )
            )
            nodes.append(
                ParagraphNode(text="Módulo de resumen ejecutivo no disponible.")
            )
        ...
        if intro_path.exists():
            ...
            nodes.append(
                ParagraphNode(text="Módulo de introducción y alcance no disponible.")
            )
        ...
        nodes.append(
            HeadingNode(
                text=f"3. {vocab.get('radar_title', 'Perfil de Madurez Ponderado')}",
                level=1,
                primary_color_rgb=self.primary_color_rgb,
            )
        )
        ...
        nodes.append(
            HeadingNode(
                text=vocab.get("dashboard_title", "Cuadro de Mando de Madurez"),
                level=2,
                primary_color_rgb=self.primary_color_rgb,
            )
        )
        nodes.append(
            ParagraphNode(
                text=vocab.get(
                    "dashboard_intro",
                    "La evaluación integral del estado actual consolida una valoración cuantitativa:",
                )
            )
        )
            ...
            TableNode(
                rows=[
                    TableRowNode(
                        cells=[
                            CellNode(
                                text=vocab.get("score_lbl", "PUNTUACIÓN AS-IS"),
                                bold=True,
                                bg_color="0072BC",
                            ...
                            CellNode(
                                text=vocab.get("maturity_lbl", "NIVEL DE MADUREZ"),
                                bold=True,
                                bg_color="0072BC",
                                text_color_rgb=[255, 255, 255],
                                align="CENTER",
                    ...
                    TableRowNode(
                        cells=[
                            CellNode(
                                text=display_score_str,
                                bold=True,
                                font_size=16,
                                bg_color="FFF3CD" if gs_val < 3.4 else "D9F2D9",
                                align="CENTER",
                            ),
                            CellNode(
                                text=global_band,
        ...
        nodes.append(
            HeadingNode(
                text=vocab.get("radar_title", "Perfil Visual de Madurez"),
                level=2,
                primary_color_rgb=self.primary_color_rgb,
            )
        )
        ...
        nodes.append(
            HeadingNode(
                text=vocab.get(
                    "platform_overview_title",
                    "4. Descripción de la Plataforma de Infraestructura Actual",
                ),
                level=1,
                primary_color_rgb=self.primary_color_rgb,
            )
        ...
        nodes.append(
            HeadingNode(
                text=vocab.get(
                    "platform_overview_title", "Descripción de la Plataforma"
                ),
                level=2,
                primary_color_rgb=self.primary_color_rgb,
            )
        ...
        if desc_path.exists():
            ...
            nodes.append(
                ParagraphNode(text="Módulo de descripción de plataforma no disponible.")
            )
        ...
        nodes.append(
            HeadingNode(
                text=vocab.get(
                    "risk_matrix_title", "5. Matriz de Riesgo Cuantitativa (FAIR)"
                ),
                level=1,
                primary_color_rgb=self.primary_color_rgb,
            )
        ...
        formatted_ale = (
            f"{total_fair_ale:,.0f}".replace(",", "X")
            .replace(".", ",")
            .replace("X", ".")
            if meta_lang == "es"
            else f"{total_fair_ale:,.0f}"
        )
        risk_intro = (
        ...
        if risks_data_list:
            nodes.append(
                HeadingNode(
                    text=vocab.get(
                        "exposure_summary_title",
                        "Resumen de Exposición y Mapa de Calor",
                    ),
                    level=2,
                    primary_color_rgb=self.primary_color_rgb,
                )
            ...
            for tef_val in range(5, 0, -1):
                ...
                for lm_val in range(1, 6):
                    ...
                    row_cells.append(
                        CellNode(
                            text=cell_text, bold=True, bg_color=bg_color, align="CENTER"
                        )
                    )
            ...
            tef_headers = vocab.get("tef_headers") or [
                "Nivel TEF",
                "Denominación",
                "Frecuencia Anualizada Estimada",
            ]
            tef_data = [
                (
                    "TEF 1",
                    "Muy Bajo" if meta_lang == "es" else "Very Low",
                ...
                (
                    "TEF 3",
                    "Medio" if meta_lang == "es" else "Medium",
                ...
                (
                    "TEF 4",
                    "Alto" if meta_lang == "es" else "High",
                ...
                (
                    "TEF 5",
                    "Muy Alto" if meta_lang == "es" else "Very High",
            ...
            nodes.append(
                HeadingNode(
                    text=vocab.get(
                        "lm_title",
                        "Criterios de Magnitud de Pérdida Directa/Indirecta (LM)",
                    ),
                    level=4,
                    primary_color_rgb=self.primary_color_rgb,
                )
            )
            lm_headers = vocab.get("lm_headers") or [
                "Nivel LM",
            ...
            lm_rows = [
                TableRowNode(
                    cells=[
                        CellNode(
                            text=f"{h} ({currency})" if h_idx > 0 else h,
                            bold=True,
                            bg_color="0072BC",
                            text_color_rgb=[255, 255, 255],
                            align="CENTER",
        ...
        if risks_data_list:
            nodes.append(
                HeadingNode(
                    text=vocab.get(
                        "detailed_risks_title",
                        "Registro Detallado de Hallazgos Forenses",
                    ),
                    level=2,
                    primary_color_rgb=self.primary_color_rgb,
                )
            ...
            for p_name, pilar_risks in risks_by_pilar.items():
                ...
                headers_table = vocab.get("detailed_risks_headers") or [
                    "ID",
                    "Vulnerabilidad y Evidencias",
                    "Exposición y Riesgo",
                ]
                risk_rows = [
                    TableRowNode(
        ...
        app_a_title = re.sub(
            r"^(Apéndice|Appendix)\s+[A-Z]:\s*",
            "",
            vocab.get("appendix_a_title", "Lista de Abreviaturas"),
        )
        nodes.append(
            HeadingNode(
                text=app_a_title, level=1, primary_color_rgb=self.primary_color_rgb
            )
        )
        nodes.append(
            ParagraphNode(
                text=vocab.get(
                    "appendix_a_intro",
                    "A continuación se definen los términos técnicos utilizados:",
                )
            )
        )
                ...
                gloss_rows = [
                    TableRowNode(
                        cells=[
                            ...
                            for h in vocab.get("appendix_a_headers")
                            or ["Abreviatura", "Significado / Descripción Técnica"]
                        ],
                        is_header=True,
                    )
                ]
        ...
        app_b_title = re.sub(
            r"^(Apéndice|Appendix)\s+[A-Z]:\s*",
            "",
            vocab.get("appendix_b_title", "Cláusula de Limitación de Responsabilidad"),
        )
        nodes.append(SpacerNode(points=20))
        nodes.append(
            HeadingNode(
        ...
        app_c_title = re.sub(
            ...
            "",
            vocab.get(
                "appendix_c_title", "Registro de Custodia de Fuentes de Información"
            ),
        )
        nodes.append(SpacerNode(points=20))
        nodes.append(
            HeadingNode(
                text=app_c_title, level=1, primary_color_rgb=self.primary_color_rgb
            )
        )
        nodes.append(
            ParagraphNode(
                text=vocab.get(
                    "appendix_c_intro",
                    "Para garantizar la transparencia, a continuación se listan las fuentes documentales utilizadas:",
                )
            )
        )
        ...
        if source_docs:
            ...
            src_data = [
                (
                    ...
                    f"{vocab.get('bib_cues', '')}{tower_name}{vocab.get('bib_cues_desc', '')}{client_name}.",
                ),
                (
                    "[Dossier de Contexto]",
                ...
                (
                    ...
                    f"{vocab.get('bib_minutas', '')}{client_name}{vocab.get('bib_minutas_desc', '')}",
                ),
            ]
        ...
        source_rows = [
            TableRowNode(
                cells=[
                    ...
                    for h in vocab.get("appendix_c_headers")
                    or [
        ...
        nodes.append(
            HeadingNode(
                text="Metodología", level=1, primary_color_rgb=self.primary_color_rgb
            )
        )
        ...
        nodes.append(
            HeadingNode(
                text="Metodología de Valoración de Madurez",
                level=2,
                primary_color_rgb=self.primary_color_rgb,
            )
        )
        nodes.append(
            ParagraphNode(
                text="La madurez se califica en una escala analítica del 1,00 al 5,00, donde cada nivel determina un estadio de control:"
            )
        )

        nodes.append(
            ParagraphNode(
                text="**Nivel 1 - Inicial (1.0-1.8):** Prácticas ad-hoc, inestables o dependientes del esfuerzo heroico de personas clave.",
                style="List Bullet",
            )
        )
        nodes.append(
            ParagraphNode(
                text="**Nivel 2 - Básico (1.8-2.6):** Prácticas funcionales de manera parcial o irregular, sin consistencia organizativa.",
                style="List Bullet",
            )
        )
        nodes.append(
            ParagraphNode(
                text="**Nivel 3 - Estandarizado (2.6-3.4):** Procesos formalizados e implantados de manera coherente en toda la organización.",
                style="List Bullet",
            )
        )
        nodes.append(
            ParagraphNode(
                text="**Nivel 4 - Optimizado (3.4-4.2):** Capacidades industrializadas, gobernadas predictivamente y sustentadas en métricas.",
                style="List Bullet",
            )
        )
        nodes.append(
            ParagraphNode(
                text="**Nivel 5 - Avanzado (4.2-5.0):** Procesos dinámicos impulsados por automatización inteligente y mejora continua.",
                style="List Bullet",
            )
        )
        ...
        nodes.append(
            HeadingNode(
                text="Capacidades Evaluadas y Cálculo del Score",
                level=2,
                primary_color_rgb=self.primary_color_rgb,
            )
        )
        nodes.append(
            ParagraphNode(
                text="El nivel de madurez técnica y el score global de la torre se consolidan mediante el promedio ponderado de las calificaciones obtenidas en las siguientes dimensiones y KPIs evaluados:"
            )
        )

        nodes.append(
            ParagraphNode(
                text="**Compute Foundation & Virtualization (Peso: 22%):** Evalúa de manera integral la madurez operativa y los baselines técnicos de la infraestructura, analizando críticamente factores clave como estandarización del estate de cómputo (clusters/hosts/configuración), gestión del ciclo de vida del compute (patching/upgrades/firmware), capacity & Performance Management (CPU/RAM/overcommit, saturación) y disponibilidad de la plataforma de compute (HA operable y mantenibilidad).",
                style="List Bullet",
            )
        )
        nodes.append(
            ParagraphNode(
                text="**Container Platform & Kubernetes Operations (Peso: 19%):** Evalúa de manera integral la madurez operativa y los baselines técnicos de la infraestructura, analizando críticamente factores clave como estandarización de servicios base (ingress, registry, certificados, storage classes), gestión del ciclo de vida de clusters (upgrades, compatibilidad, deprecations), seguridad base and aislamiento (RBAC, namespaces, separación de roles) y operación y soporte del clúster (runbooks, incidentes, capacidad de nodos).",
                style="List Bullet",
            )
        )
        nodes.append(
            ParagraphNode(
                text="**Hybrid Cloud Enablement & Landing Zones (Peso: 21%):** Evalúa de manera integral la madurez operativa y los baselines técnicos de la infraestructura, analizando críticamente factores clave como existencia y cobertura de Landing Zone (estructura base cloud), guardrails y políticas de gobierno técnico (policy-as-code / estándares), estandarización de patrones de despliegue híbridos (IaaS/PaaS) y control de consumo y costes (tagging, showback, límites).",
                style="List Bullet",
            )
        )
        nodes.append(
        ...
        nodes.append(
            HeadingNode(
                text="Metodología Cuantitativa de Riesgos (FAIR)",
                level=2,
                primary_color_rgb=self.primary_color_rgb,
            )
        )
        nodes.append(
            ParagraphNode(
                text="Este diagnóstico técnico aplica el estándar internacional O-FAIR (Factor Analysis of Information Risk) del *The Open Group* para modelar, cuantificar y priorizar la exposición al riesgo de infraestructura en términos financieros reales."
            )
        )
        nodes.append(
            ParagraphNode(
                text='Para evitar la "Falacia de los Promedios" (donde las estimaciones fijas e individuales distorsionan el riesgo real), el motor de la plataforma aplica un algoritmo de simulación actuarial de Monte Carlo de 10.000 iteraciones para cada hallazgo detectado, basado en los siguientes principios científicos:'
            )
        )
        ...
        nodes.append(
            ParagraphNode(
                text="1. **Estimaciones de Tres Puntos (Incertidumbre):** Cada nivel cualitativo de la auditoría (escala 1 a 5) se traduce a un rango dinámico definido por tres parámetros: Mínimo, Más Probable y Máximo.",
                bold=False,
            )
        )
        nodes.append(
            ParagraphNode(
                text="**Frecuencia de Amenaza (TEF):** Mapea la probabilidad anualizada de exposición (de 0,1 a 24 eventos de amenaza al año).",
                style="List Bullet",
            )
        )
        nodes.append(
            ParagraphNode(
                text="**Vulnerabilidad (Vuln):** Mapea la probabilidad de éxito de la amenaza según el nivel de controles (de 5% a 100%).",
                style="List Bullet",
            )
        )
        nodes.append(
            ParagraphNode(
                text=f"**Magnitud de Pérdida (LM):** Mapea el impacto financiero directo e indirecto (laboral, cumplimiento, remediación) desde 100 {currency} hasta {formatted_max_lm} por incidente.",
                style="List Bullet",
            )
        )
        ...
        nodes.append(
            ParagraphNode(
                text="3. **Cálculo del Exposición Anualizada de Pérdida (ALE):** En cada iteración se realiza un sorteo de Bernoulli basado en la vulnerabilidad muestreada. Si el evento de pérdida se materializa, el coste anualizado para esa simulación es el producto de la frecuencia por la pérdida promedio:",
                bold=False,
            )
        )
        ...
        nodes.append(
            HeadingNode(
                text="Perfil de Calibración Activo y Escala de Pérdidas",
                level=2,
                primary_color_rgb=self.primary_color_rgb,
            )
        )
        ...
        if is_critical:
            nodes.append(
                ParagraphNode(
                    text="**Perfil de Calibración Activo: CRITICAL_INFRASTRUCTURE (Escala Alta / Regulada)**",
                    style="List Bullet",
                )
            )
            nodes.append(
                ParagraphNode(
                    text=f"Debido a la naturaleza del negocio de **{client_name}** como operador de Infraestructura Crítica en el sector de Energía (o servicios globales de alta criticidad), se ha activado el Preset de calibración de alta escala. Esto ajusta los rangos de Magnitud de Pérdida (LM) de acuerdo con el impacto sistémico real de interrupciones en su plataforma core:",
                    style="List Bullet",
                )
            )
            nodes.append(
                ParagraphNode(
                    text=f"**LM 1 (Impacto Muy Bajo):** Rango de **< 1.000 {currency}**. Se fundamenta en el coste directo de intervenciones rápidas y sencillas de reconfiguración técnica (menos de 8 horas de dedicación de un ingeniero de soporte).",
                    style="List Bullet",
                )
            )
            nodes.append(
                ParagraphNode(
                    text=f"**LM 2 (Impacto Bajo):** Rango de **1.000 {currency} a 5.000 {currency}**. Se fundamenta en el coste laboral de paradas de servicio locales de menos de 3 horas que requieren la intervención de un Ingeniero Senior o Administrador de Sistemas para su remediación, sin penalizaciones por SLA de clientes.",
                    style="List Bullet",
                )
            )
            nodes.append(
                ParagraphNode(
                    text=f"**LM 3 (Impacto Medio):** Rango de **5.000 {currency} a 25.000 {currency}**. Se fundamenta en incidencias de seguridad de severidad media, caída de entornos de pre-producción o indisponibilidades que afecten temporalmente la productividad de equipos internos de ingeniería por un máximo de 24 horas laborables.",
                    style="List Bullet",
                )
            )
            nodes.append(
                ParagraphNode(
                    text=f"**LM 4 (Impacto Alto):** Rango de **50.000 {currency} a 300.000 {currency}**. Este umbral se fundamenta de forma estricta en el régimen sancionador de la directiva europea **NIS2** y el **Esquema Nacional de Seguridad (ENS)** en España para incidentes graves derivados de falta de mantenimiento u obsolescencia técnica.",
                    style="List Bullet",
                )
            )
            nodes.append(
                ParagraphNode(
                    text=f"**LM 5 (Impacto Muy Alto):** Rango de **250.000 {currency} a 1.500.000 {currency}**. Este rango está alineado con el estudio de referencia mundial **ITIC Global Server Hardware Downtime Report**, el cual determina que para el 98% de los operadores de infraestructuras críticas y energía, el coste de una sola hora de indisponibilidad de sistemas core supera el millón de euros debido a la interrupción de procesos, penalizaciones por acuerdos de nivel de servicio (SLA) y remediación forense urgente.",
                    style="List Bullet",
                )
            )
        else:
            nodes.append(
                ParagraphNode(
                    text="**Perfil de Calibración Activo: MID_MARKET (Escala Organizacional Media)**",
                    style="List Bullet",
                )
            )
            nodes.append(
                ParagraphNode(
                    text=f"Debido a la escala organizativa de **{client_name}**, el motor de riesgos de la plataforma ha activado el Preset de calibración del mercado medio. Esto dimensiona las pérdidas a un rango adecuado al impacto de su negocio:",
                    style="List Bullet",
                )
            )
            nodes.append(
                ParagraphNode(
                    text=f"**LM 1 (Impacto Muy Bajo):** Rango de **< 1.000 {currency}**. Se fundamenta en el coste directo de reconfiguraciones técnicas rápidas de bajo impacto técnico.",
                    style="List Bullet",
                )
            )
            nodes.append(
                ParagraphNode(
                    text=f"**LM 2 (Impacto Bajo):** Rango de **1.000 {currency} a 5.000 {currency}**. Basado en incidencias menores solucionables por el equipo de soporte de primer nivel en pocas horas sin afectación de facturación.",
                    style="List Bullet",
                )
            )
            nodes.append(
                ParagraphNode(
                    text=f"**LM 3 (Impacto Medio):** Rango de **5.000 {currency} a 25.000 {currency}**. Estimando fallos parciales de disponibilidad que afecten temporalmente la productividad de equipos técnicos de soporte interno.",
                    style="List Bullet",
                )
            )
            nodes.append(
                ParagraphNode(
                    text=f"**LM 4 (Impacto Alto):** Rango de **25.000 {currency} a 100.000 {currency}**, estimando interrupciones de servicios no críticos y costes de auditorías de cumplimiento extraordinarias.",
                    style="List Bullet",
                )
            )
            nodes.append(
                ParagraphNode(
                    text=f"**LM 5 (Impacto Muy Alto):** Rango de **100.000 {currency} a 500.000 {currency}**, basado en la parada de la cadena de facturación o producción durante más de 8 horas laborables consecutivas.",
                    style="List Bullet",
                )
            )

        return DocumentAST(
            title=f"Technical Annex - Tower {tower_id}",
            metadata=tower_meta,
            nodes=nodes,
        )

    def _parse_markdown_to_nodes(
        self, md_path: Path, skip_level_1: bool = False
    ) -> List[Any]:
        ...
        for line in lines:
            ...
            #
                ...
                nodes.append(
                    ParagraphNode(text=cleaned[2:], style="List Bullet", space_after=6)
                )
            #
            elif cleaned.startswith("> "):
                nodes.append(
                    ParagraphNode(text=cleaned[2:], italic=True, space_after=12)
                )
            #
            else:
    ...
    def _extract_key_bullets_from_md(
        self, md_path: Path, section_marker: str
    ) -> List[str]:
            ...
            for r_idx, row_data in enumerate(reader):
                ...
                for c_idx, val in enumerate(row_data):
                    ...
                    align = (
                        "CENTER"
                        if c_idx == 1
                        else ("LEFT" if c_idx == 2 else "JUSTIFY")
                    )
                    cells.append(
                        CellNode(text=val, bold=bold, bg_color=bg, align=align)
                    )
