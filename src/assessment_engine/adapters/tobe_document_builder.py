...
def build_master_ast(working_dir: str, output_path: str):
    ...
    for file_path in sorted(glob.glob(search_pattern)):
        with open(file_path, "r", encoding="utf-8-sig") as f:
            try:
                ...
                print(
                    f"   ├─ Cargada torre: {data.get('document_meta', {}).get('tower_name', 'TXX')}"
                )
    ...
    global_summary_path = os.path.join(
        working_dir, "global_tobe_executive_summary.json"
    )
    global_summary = {}
    if os.path.exists(global_summary_path):
        ...
        print(
            "   ⚠️ Sumario ejecutivo global no encontrado. El AST se generará sin resumen general."
        )
    ...
    for payload in payloads:
        ...
        t_scores = [
            p.get("target_score", 4.0) for p in payload.get("pillars_analysis", [])
        ]
        ...
        for pilar in payload.get("pillars_analysis", []):
            ...
            p_data = {
                ...
                "asis_description": pilar.get(
                    "asis_architecture_description", "No descripto."
                ),
                "score": pilar.get("score", 0.0),
                "target_score": pilar.get("target_score", 4.0),
                "vision_3_years": tobe_raw.get(
                    "vision_3_years", tobe_raw.get("vision", "No definido.")
                ),
    ...
    out_path.write_text(
        json.dumps(master_ast, indent=4, ensure_ascii=False), encoding="utf-8-sig"
    )
