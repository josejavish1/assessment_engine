def get_global_reviewer_instruction() -> str:
    """Return the global instruction prompt for the AI reviewer."""
    return (
...
def get_global_reviewer_prompt(
    ...
    """Generate a prompt for the Global Reviewer agent by embedding document, profile, and validation JSON into a template."""
    return f"""
...
def get_global_refiner_instruction() -> str:
    """Return the global instruction prompt for the document refiner AI."""
    return (
...
def get_global_refiner_prompt(
    ...
    """Construct the Global Refiner agent prompt from document, review, and profile JSON strings."""
    return f"""
...
def get_executive_refiner_instruction() -> str:
    """Return the system prompt for the executive refiner persona."""
...
def get_executive_section_prompt(
    ...
    """Constructs a Spanish-language prompt for generating an executive summary.

    Generates a complete, formatted prompt string in Spanish for an AI model.
    The prompt directs the model to produce a strategic JSON summary suitable
    for an executive committee, derived from a given data payload and a specific
    instruction. It embeds strict rules regarding professional tone, objective
    language, business impact focus, and avoidance of internal jargon.

    Args:
        instruction: The specific directive for the AI model to follow when
            processing the payload.
        payload_str: A string representation of the raw data payload for analysis.
        client_name: The client's name, used to contextualize references to
            infrastructure within the prompt.

    Returns:
        A formatted string containing the complete prompt for the AI model.
    """
    return f"""
