...
class EpistemicGraphService:
    ...
    def __init__(self, industry_profile: str = "default") -> None:
        ...
        dependencies = [
            ...
            ("T5", "T2", 0.85),  # Cybersecurity (T5) protects and influences OT Substation Systems (T2)
            ("T3", "T2", 0.70),  # Core Network (T3) directly impacts OT Substation Systems (T2)
            ("T3", "T8", 0.65),  # Core Network (T3) hosts and impacts Cloud/Datacenter Infrastructure (T8)
            ("T8", "T4", 0.80),  # Cloud/DC Infrastructure (T8) hosts and impacts Core Business Apps (T4)
            ("T9", "T1", 0.55),  # Physical Security (T9) impacts End-User Computing (T1)
            ("T4", "T10", 0.60), # Business Apps (T4) provide data for Digital Twin Operations (T10)
            ("T6", "T1", 0.50),  # IAM (T6) governs and impacts End-User Computing (T1)
            ("T5", "T3", 0.50),  # SOC (T5) filters and secures Core Networks (T3) - Cycle T3 <-> T5!
            ("T8", "T6", 0.45),  # Cloud (T8) hosts and impacts IAM (T6) - Cycle T6 <-> T8!
        ]

        # Attempt to load declarative topology based on industry profile
        try:
            repo_root = Path(__file__).resolve().parent.parent.parent.parent
            policy_file = repo_root / "engine_config" / "policies" / "epistemic_dependencies.json"
            ...
            if policy_file.exists():
                ...
                if "energy" in self.industry_profile or "critical" in self.industry_profile:
                ...
                if profile_deps:
                    ...
                    logger.info(f"Loaded declarative epistemic topology for profile: {target_profile}")
        except Exception as e:
            logger.warning(f"Failed to load declarative epistemic policy ({e}). Falling back to robust expert defaults.")

        # Build the graph edges (supports loops natively)
        for u, v, w in dependencies:
            self.graph.add_edge(u, v, weight=w)

    def propagate_risk(self, intrinsic_maturities: Dict[str, float], max_iter: int = 100, tol: float = 1e-6) -> Dict[str, float]:
    ...
    def get_single_point_of_failure(self, intrinsic_maturities: Dict[str, float]) -> Tuple[str, float]:
        ...
        for t_id in self.towers:
            ...
            perturbed_sum_risk = sum(1.0 - (v - 3.0)/2.0 for v in perturbed_propagated.values())
    ...
    def backpropagate_audit_feedback(
        ...
        R_int = np.array([1.0 - (intrinsic_maturities.get(t, 4.0) - 3.0)/2.0 for t in self.towers])
        R_aud = np.array([1.0 - (audited_maturities.get(t, 4.0) - 3.0)/2.0 for t in self.towers])
                ...
                if self.graph.has_edge(u, v):
                    ...
                    for k_idx, k in enumerate(self.towers):
                        if k != u and self.graph.has_edge(k, v):
                            parent_safety_product *= (1.0 - self.graph[k][v]["weight"] * R_prop[k_idx])
                            
                    d_rprop_dw = (1.0 - R_int[v_idx]) * R_prop[u_idx] * parent_safety_product
        ...
        try:
            ...
            if policy_file.exists():
                ...
                if "energy" in self.industry_profile or "critical" in self.industry_profile:
                    ...
                    target_profile = "retail"
                elif self.industry_profile in policy_data.get("industry_topologies", {}):
                    target_profile = self.industry_profile
