    ...
    def _update_terminal_mirror(self, lines: list[str]):
        ...
        for line in lines:
            ...
            if line_clean:
                ...
                # Feed Brain Scanner from session log
                if "Function call(s) received" in line_clean:
                    brain.write("[bold cyan][AGENT][/] Thinking... requesting tools.")
                elif "Executed tool 'list_source_files'" in line_clean:
                    brain.write("[bold green][AGENT][/] Exploring repository files...")
                elif "Executed tool 'read_doc_file'" in line_clean or "list_architecture_docs" in line_clean:
                    brain.write("[bold green][AGENT][/] Reading architecture documentation...")
                elif "Executed tool 'inspect_module'" in line_clean:
                    brain.write("[bold green][AGENT][/] Inspecting code module...")
                elif "Executed tool 'replace'" in line_clean or "write_file" in line_clean:
                    brain.write("[bold magenta][AGENT][/] Applying code changes...")
                elif "Feeding tool results back" in line_clean:
                    brain.write("[bold yellow][AGENT][/] Processing tool results...")
