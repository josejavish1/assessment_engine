...
def validate_contracts_schemas(
    ...
    for entry in documentation_map.get("entries", []):
        ...
        try:
            ...
            errors.append(f"{path}: failed to import source of truth module for {python_source.name}: {err}")
            continue
        ...
        for class_name, fields in extracted_models.items():
            ...
            documented_field_names = {f["field_name"] for f in fields}
            ...
            for p_field_name, p_field_def in pydantic_fields.items():
                ...
                for f in fields:
                    ...
                    if f["alias"] and f["alias"] == p_field_name:
                        has_documented = True
                        break
                    if p_field_def.alias and (p_field_def.alias == f["field_name"] or p_field_def.alias == f["alias"]):
                        has_documented = True
                        break
            ...
            for f in fields:
                ...
                if not p_field_def:
                    ...
                    for pf_name, pf_def in pydantic_fields.items():
                        if pf_def.alias == f_name or pf_def.alias == f["alias"] or pf_name == f["alias"]:
                            p_field_def = pf_def
                            break
