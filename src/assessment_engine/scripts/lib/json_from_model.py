...
def parse_json_from_text(text: str):
    ...
    if not starts:
        raise json.JSONDecodeError("No JSON object/array found in model output", cleaned, 0)
    ...
    if end == -1 or end <= start:
        raise json.JSONDecodeError("Incomplete JSON block in model output", cleaned, start)
