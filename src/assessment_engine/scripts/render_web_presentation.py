"""
Módulo render_web_presentation.py.
...
def generate_web_dashboard(client_id):
    base_dir = Path(f"assessment_engine/working/{client_id}")
    global_payload_path = base_dir / "global_report_payload.json"

    if not global_payload_path.exists():
        print("Error: No se encuentra el payload global")
        return

    with open(global_payload_path, "r", encoding="utf-8-sig") as f:
    ...
    nexus_data = {
        ...
        "strategy": {
            "headline": global_raw.get("executive_summary", {}).get("headline", ""),
            "narrative": global_raw.get("executive_summary", {}).get("narrative", ""),
            "global_score": global_raw.get("executive_summary", {}).get("score", "2.6"),
            "burning_platform": global_raw.get("burning_platform", []),
            "principles": global_raw.get("target_vision", {}).get(
                "evolution_principles", []
            ),
            "architecture_principles": global_raw.get("target_vision", {}).get(
                "architecture_principles", []
            ),
            "operating_model": global_raw.get("target_vision", {}).get(
                "operating_model_implications", []
            ),
            "gtm_strategy": global_raw.get("gtm_strategy", {}),
            "stakeholders": global_raw.get("stakeholder_matrix", []),
            "business_impacts": global_raw.get("executive_summary", {}).get(
                "key_business_impacts", []
            ),
            "estimated_tam": global_raw.get("meta", {}).get("estimated_tam", "TBD"),
        },
        "roadmap": {
            "programs": global_raw.get("execution_roadmap", {}).get("programs", []),
            "horizons": global_raw.get("execution_roadmap", {}).get("horizons", {}),
            "proactive_proposals": global_raw.get("execution_roadmap", {}).get(
                "proactive_proposals", []
            )
            or [
                {
                    "initiative_name": "Programa de Choque NIS2 & Ciber-Resiliencia",
                    "executive_synthesis": "Plan de remediación acelerada para cerrar las brechas críticas de seguridad perimetral, desplegar inmutabilidad de backups frente a ransomware y establecer un marco de gobierno auditable.",
                    "context_and_why": {
                        "cost_of_inaction": "Incumplimiento de normativas (NIS2/GDPR) con riesgo de sanciones millonarias e incapacidad de recuperar las operaciones ante un ataque."
                    },
                    "solution_and_what": {
                        "target_state": "Postura defensiva validada, MFA ubicuo, y capacidad demostrable de recuperación (Cyber Vault)."
                    },
                    ...
                    "deep_dive": {
                        ...
                        "roi": "Evita multas regulatorias de hasta el 2% de la facturación global y mitiga el lucro cesante por paradas operativas superiores a 24h.",
                        "prerequisites": "Aprobación de presupuesto CAPEX extraordinario y disponibilidad del equipo de cumplimiento corporativo.",
                    },
                },
                {
                    "initiative_name": "Estabilización Core Network & SD-WAN",
                    "executive_synthesis": "Saneamiento de la arquitectura de red corporativa para eliminar puntos únicos de fallo (SPOF) y migrar hacia un modelo SD-WAN que garantice el rendimiento de aplicaciones críticas.",
                    "context_and_why": {
                        "cost_of_inaction": "Caídas de servicio impredecibles y lentitud que paraliza la operativa del personal y la atención clínica/cliente."
                    },
                    "solution_and_what": {
                        "target_state": "Red de alta disponibilidad, monitorización proactiva y ancho de banda garantizado para SaaS."
                    },
                    ...
                    "deep_dive": {
                        ...
                        "roi": "Reducción del 80% en los tiempos de caída de sedes. Incremento directo en la productividad de los empleados y mejora en la experiencia del paciente.",
                        "prerequisites": "Ventanas de mantenimiento aprobadas para cortes controlados durante la migración de la electrónica.",
                    },
                },
            ],
        },
        "heatmap": global_raw.get("heatmap", []),
        "towers": {},
    }

    for tower_meta in global_raw.get("heatmap", []):
        t_id = tower_meta.get("id")
        t_dir = base_dir / t_id

        # Lógica de fallback inteligente para complejidad si no está en el blueprint
        try:
            curr_score = float(tower_meta.get("score", "2.5"))
        except Exception:
            curr_score = 2.5

        derived_complexity = (
            "Alta" if curr_score < 2.2 else ("Media" if curr_score < 3.2 else "Baja")
        )
        ...
        tower_nexus = {
            ...
            "decisions": [],
            "cost_of_inaction": "",
            "target_maturity": tower_meta.get("target_maturity", "4.0"),
            "complexity": derived_complexity,
        }

        bp_path = t_dir / f"blueprint_{t_id.lower()}_payload.json"
        if bp_path.exists():
            with open(bp_path, "r", encoding="utf-8-sig") as f:
                bp = json.load(f)
                snap = bp.get("executive_snapshot", {})

                # Sanitizar Complejidad: Extraer solo la primera palabra (Alta, Media, Baja)
                raw_complexity = snap.get(
                    "transformation_complexity", tower_nexus["complexity"]
                )
                if "Alta" in raw_complexity:
                    tower_nexus["complexity"] = "Alta"
                elif "Media" in raw_complexity:
                    tower_nexus["complexity"] = "Media"
                elif "Baja" in raw_complexity:
                    tower_nexus["complexity"] = "Baja"
                else:
                    tower_nexus["complexity"] = raw_complexity[:10]  # Fallback corto

                # Buscar Madurez Objetivo real en el blueprint de la torre (puede estar en varios sitios)
                bp_target = None
                # Intento 1: En el resumen de madurez del blueprint
                if bp.get("summary", {}).get("target_maturity"):
                    bp_target = bp.get("summary", {}).get("target_maturity")
                # Intento 2: En la visión tobe del blueprint
                elif bp.get("target_architecture_tobe", {}).get("target_maturity"):
                    target_obj = bp.get("target_architecture_tobe", {}).get(
                        "target_maturity"
                    )
                    if isinstance(target_obj, dict):
                        bp_target = target_obj.get("recommended_score_reference")

                if bp_target:
                    tower_nexus["target_maturity"] = bp_target

                tower_nexus.update(
                    {
                        "executive_summary": snap.get(
                            "bottom_line", tower_nexus["executive_summary"]
                        ),
                        "cost_of_inaction": snap.get("cost_of_inaction", ""),
                        "decisions": snap.get("decisions", []),
                        "structural_risks": snap.get("structural_risks", []),
                        "business_impact": snap.get("business_impact", ""),
                        "operational_benefits": snap.get("operational_benefits", []),
                    }
                )
                for p in bp.get("pillars_analysis", []):
                    tower_nexus["pillars"].append(
                        {
                            "name": p.get("pilar_name"),
                            "score": p.get("score"),
                            "health_check": p.get("health_check_asis", []),
                            "projects": p.get("projects_todo", []),
                            "target_vision": p.get("target_architecture_tobe", {}).get(
                                "vision", ""
                            ),
                        }
                    )
        nexus_data["towers"][t_id] = tower_nexus

    pres_dir = base_dir / "presentation"
    pres_dir.mkdir(parents=True, exist_ok=True)
    json_data = json.dumps(nexus_data).replace("</script>", "<\\/script>")
...
<head>
    ...
    <title>Strategic Dashboard v5.1 | __CLIENT_ID__</title>
    ...
    <style>
        ...
        body { background-color: #020617; color: #f8fafc; font-family: 'Plus Jakarta Sans', sans-serif; overflow: hidden; height: 100vh; }
        .glass { background: rgba(15, 23, 42, 0.7); border: 1px solid rgba(255,255,255,0.08); backdrop-filter: blur(16px); }
        .tower-card, .proj-tile, .pilar-row, .prog-card { cursor: pointer; transition: all 0.2s ease; }
        .tower-card:hover, .prog-card:hover { transform: translateY(-4px); border-color: #38bdf8; background: rgba(15, 23, 42, 0.9); }
        .proj-tile:hover { background: rgba(56, 189, 248, 0.1); border-color: #38bdf8; }
        .custom-scrollbar::-webkit-scrollbar { width: 4px; }
        .custom-scrollbar::-webkit-scrollbar-thumb { background: #334155; border-radius: 10px; }
        .mono { font-family: 'JetBrains Mono', monospace; }
        .blueprint-panel { max-height: 0; overflow: hidden; transition: max-height 0.4s ease; }
        .blueprint-panel.open { max-height: 3000px; }
        .app-container { height: 100vh; display: flex; flex-direction: column; }
        .content-scroll { flex: 1; overflow-y: auto; scroll-behavior: smooth; }
        .sidebar-scroll { height: calc(100vh - 250px); overflow-y: auto; }
        
        /* Interactive Highlight Classes */
        .dimmed { opacity: 0.2; filter: grayscale(0.8); }
        .highlighted { border-color: #38bdf8 !important; box-shadow: 0 0 20px rgba(56, 189, 248, 0.3); transform: scale(1.02); }
        
        /* Gantt Mini-Chart */
        .gantt-bar { height: 8px; border-radius: 4px; position: relative; }
        .gantt-label { font-size: 8px; font-weight: bold; color: #64748b; margin-top: 4px; }
    </style>
</head>
<body>

    <div class="app-container p-4 md:p-8 space-y-8">
        <!-- HEADER -->
        <header class="flex flex-col lg:flex-row justify-between items-center glass p-8 rounded-3xl border-white/5 shadow-2xl shrink-0">
            <div class="flex items-center gap-8">
                <h1 class="text-5xl font-black text-white tracking-tighter uppercase leading-none">__CLIENT_ID__</h1>
                <div class="h-12 w-[1px] bg-slate-700"></div>
                <div>
                    <span class="text-[10px] font-black text-sky-500 uppercase tracking-widest block mb-1 italic">Strategic Insights & Transformation Engine</span>
                    <h2 id="ui-headline" class="text-xl text-slate-300 font-medium italic truncate max-w-3xl"></h2>
                </div>
            </div>
            <div class="flex items-center gap-8 bg-slate-900/60 p-4 px-8 rounded-2xl border border-white/10">
                <div class="text-center">
                    <p class="text-[9px] text-slate-500 font-black uppercase mb-1 tracking-widest">Global score</p>
                    <div id="ui-score" class="text-5xl font-black text-sky-400 mono">--</div>
                </div>
            </div>
        </header>

        <div class="grid grid-cols-1 xl:grid-cols-12 gap-8 overflow-hidden flex-1">
            <!-- SIDEBAR -->
            <div class="xl:col-span-4 space-y-6 sidebar-scroll custom-scrollbar pr-2">
                <div class="glass p-6 rounded-3xl">
                    <h2 class="text-xs font-black text-slate-500 uppercase tracking-[0.4em] text-center mb-6 border-b border-white/5 pb-4">Madurez de Torres</h2>
                    <div class="w-full aspect-square relative flex items-center justify-center"><canvas id="globalRadarChart"></canvas></div>
                </div>
                
                <div class="glass p-8 rounded-3xl border-l-8 border-l-rose-600 shadow-xl">
                    <h2 class="text-base font-black text-white mb-6 uppercase tracking-widest flex items-center gap-4"><i class="fa-solid fa-fire text-rose-500"></i> Burning Platform</h2>
                    <div id="ui-burning-platform" class="space-y-8"></div>
                </div>

                <!-- NEW SECTION: OPERATING MODEL & PRINCIPLES -->
                <div class="glass p-8 rounded-3xl border-l-8 border-l-indigo-600 shadow-xl">
                    <h2 class="text-base font-black text-white mb-6 uppercase tracking-widest flex items-center gap-4"><i class="fa-solid fa-gears text-indigo-500"></i> Modelo Operativo</h2>
                    <div id="ui-op-model" class="space-y-4"></div>
                </div>
            </div>

            <!-- MAIN CONTENT -->
            <div class="xl:col-span-8 space-y-8 content-scroll custom-scrollbar pr-4">
                
                <!-- EXECUTIVE HIGHLIGHTS SECTION (Moved here for scrolling) -->
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 shrink-0">
                    <div class="lg:col-span-6 glass p-10 rounded-[2.5rem] border-l-[12px] border-l-sky-500 shadow-xl">
                        <h3 class="text-[10px] font-black text-sky-400 uppercase tracking-[0.3em] mb-6">Síntesis Ejecutiva Global</h3>
                        <p id="ui-narrative" class="text-[11px] text-slate-300 leading-relaxed font-medium italic text-justify"></p>
                    </div>
                    <div class="lg:col-span-6 glass p-10 rounded-[2.5rem] border-l-8 border-l-emerald-500 shadow-xl">
                        <h3 class="text-[10px] font-black text-emerald-400 uppercase tracking-[0.3em] mb-6">Impactos Críticos de Negocio</h3>
                        <ul id="ui-business-impacts" class="space-y-4"></ul>
                    </div>
                </div>

                <!-- Heatmap -->
                <div class="glass p-10 rounded-[3rem] shadow-xl">
                    <h2 class="text-2xl font-black text-white mb-8 tracking-tight uppercase">Capacidades por Torre</h2>
                    <div id="ui-heatmap" class="grid grid-cols-1 md:grid-cols-3 gap-6"></div>
                </div>

                <!-- Strategic Programs -->
                <div class="glass p-10 rounded-[3rem] shadow-xl">
                    <h2 class="text-2xl font-black text-white mb-8 tracking-tight flex items-center gap-4 uppercase"><i class="fa-solid fa-layer-group text-indigo-400"></i> Programas de Transformación</h2>
                    <p class="text-slate-500 text-xs mb-6 -mt-4 uppercase tracking-widest">Haz clic en un programa para filtrar el roadmap</p>
                    <div id="ui-programs" class="grid grid-cols-1 md:grid-cols-3 gap-6"></div>
                </div>

                <!-- Roadmap -->
                <div class="glass p-10 rounded-[3rem] shadow-xl overflow-hidden">
                    <h2 class="text-2xl font-black text-white mb-10 tracking-tight flex items-center gap-4 uppercase"><i class="fa-solid fa-timeline text-sky-500"></i> Roadmap de Ejecución Estratégica</h2>
                    <div id="ui-roadmap" class="space-y-16"></div>
                </div>

                <!-- Sankey (Nexo de Impacto) -->
                <div class="glass p-10 rounded-[3rem] shadow-xl overflow-hidden">
                    <h2 class="text-2xl font-black text-white mb-4 tracking-tight flex items-center gap-4 uppercase"><i class="fa-solid fa-diagram-project text-emerald-400"></i> Nexo de Impacto Estratégico</h2>
                    <p class="text-slate-500 text-xs mb-8 uppercase tracking-widest">Visualización de flujos: Gaps de Torre → Proyectos → Impacto de Negocio</p>
                    <div id="sankey-container" class="w-full h-[600px] bg-slate-950/50 rounded-2xl border border-white/5 relative">
                ...
                <div class="glass p-10 rounded-[3rem] shadow-xl">
                    <h2 class="text-2xl font-black text-white mb-8 tracking-tight flex items-center gap-4 uppercase"><i class="fa-solid fa-rocket text-sky-400"></i> Command Center: Propuestas Proactivas</h2>
                    <div id="ui-proposals" class="grid grid-cols-1 gap-8"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- TOWER MODAL -->
    <div id="tower-modal" class="fixed inset-0 z-50 hidden bg-slate-950/98 backdrop-blur-3xl flex justify-center items-center p-4 md:p-8 opacity-0 transition-all duration-500">
        <div class="bg-[#0b1222] border border-white/10 rounded-3xl w-full max-w-[1650px] h-full max-h-[92vh] flex flex-col shadow-[0_0_100px_rgba(0,0,0,0.8)] transform scale-95 transition-all duration-500 overflow-hidden" id="modal-content">
            <div class="p-10 border-b border-white/5 flex justify-between items-center bg-slate-900/40">
                <div class="flex items-center gap-10">
                    <div id="modal-score-box" class="w-24 h-24 rounded-3xl flex flex-col items-center justify-center border-2 border-white/10 bg-slate-950/50 shadow-inner">
                        <span class="text-[9px] text-slate-500 font-black uppercase mb-1">Score</span>
                        <span id="modal-tower-score" class="text-5xl font-black mono tracking-tighter">--</span>
                    </div>
                    <div>
                        <div class="flex items-center gap-5 mb-2">
                            <span id="modal-tower-id" class="px-3 py-1 bg-sky-500 text-slate-950 text-[11px] font-black rounded-lg uppercase"></span>
                            <h2 id="modal-tower-name" class="text-5xl font-extrabold text-white tracking-tighter uppercase leading-none"></h2>
                        </div>
                        <span id="modal-tower-band" class="text-sm font-black tracking-[0.3em] uppercase text-slate-500"></span>
                    </div>
                </div>
                <button onclick="closeModal()" class="w-16 h-16 rounded-[2rem] bg-white/5 hover:bg-rose-500/20 text-slate-400 hover:text-rose-500 transition-all flex items-center justify-center border border-white/5"><i class="fa-solid fa-xmark text-3xl"></i></button>
            </div>
            <div class="p-12 overflow-y-auto custom-scrollbar flex-1">
                <div class="space-y-12 max-w-[1600px] mx-auto">
                    ...
                    <div class="grid grid-cols-1 xl:grid-cols-2 gap-10 items-stretch">
                        
                        <!-- LEFT COLUMN: THE PROBLEM -->
                        <div class="space-y-8 flex flex-col">
                            <div class="glass p-10 rounded-[2.5rem] border-l-[12px] border-l-sky-500 shadow-xl flex-1 flex flex-col">
                                <h3 class="text-[10px] font-black text-sky-400 uppercase tracking-[0.3em] mb-6 italic">Bottom Line & Síntesis</h3>
                                <p id="modal-narrative" class="text-sm text-slate-300 leading-relaxed font-medium text-justify"></p>
                                <div class="mt-auto pt-8 border-t border-white/5">
                                    <h4 class="text-[9px] font-black text-emerald-400 uppercase tracking-widest mb-3">Impacto en Negocio</h4>
                                    <p id="modal-business-impact" class="text-[11px] text-slate-400 italic leading-snug"></p>
                                </div>
                            </div>
                            
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div class="glass p-10 rounded-[2.5rem] border-l-8 border-l-rose-600 bg-rose-500/5 shadow-xl h-full">
                                    <h4 class="text-xs font-black text-rose-400 uppercase mb-6 tracking-widest flex items-center gap-3"><i class="fa-solid fa-triangle-exclamation"></i> Riesgos Estructurales</h4>
                                    <ul id="modal-structural-risks" class="text-[11px] text-slate-300 space-y-4 list-none"></ul>
                                </div>
                                <div class="glass p-10 rounded-[2.5rem] border-l-8 border-l-rose-500 shadow-xl h-full">
                                    <h4 class="text-xs font-black text-rose-400 uppercase mb-6 tracking-widest flex items-center gap-2"><i class="fa-solid fa-biohazard"></i> Coste de Inacción</h4>
                                    <p id="modal-coi" class="text-sm text-rose-100/90 leading-relaxed font-medium text-justify"></p>
                                </div>
                            </div>
                        </div>

                        <!-- RIGHT COLUMN: THE ANALYSIS & SOLUTION -->
                        <div class="space-y-8 flex flex-col">
                            <div class="glass p-10 rounded-[2.5rem] border border-white/5 flex flex-col bg-slate-950/40 relative">
                                <h3 class="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 text-center">Métricas de Madurez de Capacidad</h3>
                                <div class="w-full h-[400px] xl:h-[450px]"><canvas id="towerRadarChart"></canvas></div>
                            </div>
                            
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div class="glass p-10 rounded-[2.5rem] border-l-8 border-l-indigo-500 shadow-xl bg-indigo-500/5 h-full">
                                    <h3 class="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-4">Visión To-Be</h3>
                                    <p id="modal-vision-text" class="text-[11px] text-slate-300 italic leading-relaxed"></p>
                                </div>
                                <div class="glass p-10 rounded-[2.5rem] border-l-8 border-l-emerald-500 bg-emerald-500/5 shadow-xl h-full">
                                    <h4 class="text-xs font-black text-emerald-400 uppercase mb-6 tracking-widest flex items-center gap-3"><i class="fa-solid fa-bolt"></i> Beneficios Operativos</h4>
                                    <ul id="modal-op-benefits" class="text-[11px] text-slate-300 space-y-4 list-none"></ul>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- FULL WIDTH SECTION: DECISIONS, GANTT & PILLARS -->
                    <div class="space-y-12">
                        
                        <!-- DECISIONS BOARD (FULL WIDTH) -->
                        <div class="glass p-10 rounded-[2.5rem] border-l-8 border-l-amber-500 shadow-xl bg-amber-500/5">
                            <h4 class="text-sm font-black text-amber-400 uppercase mb-6 tracking-widest flex items-center gap-3"><i class="fa-solid fa-gavel"></i> Decisiones Estratégicas Board</h4>
                            <ul id="modal-decisions" class="text-sm text-slate-200 space-y-4 list-none grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"></ul>
                        </div>

                        <!-- GANTT AREA (FULL WIDTH) -->
                        <div class="glass p-12 rounded-[2.5rem] shadow-2xl border border-white/5 bg-slate-900/40">
                            <div class="flex justify-between items-center mb-12">
                                <h3 class="text-xl font-black text-sky-400 uppercase tracking-widest flex items-center gap-4"><i class="fa-solid fa-timeline"></i> Engineering Execution Roadmap</h3>
                                <span class="text-xs font-black text-slate-500 uppercase tracking-widest px-4 py-2 border border-slate-700 rounded-lg bg-slate-800/50">Escala: 36 Meses</span>
                            </div>
                            <div id="modal-gantt" class="space-y-5"></div>
                        </div>

                        <!-- PILLARS BREAKDOWN (FULL WIDTH) -->
                        <div class="space-y-8 pt-8">
                            <h3 class="text-3xl font-black text-white flex items-center gap-4 uppercase tracking-tight mb-8"><i class="fa-solid fa-layer-group text-sky-500"></i> Desglose Detallado por Capacidad</h3>
                            <div id="modal-pillars-rows" class="space-y-8"></div>
                        </div>
    ...
    <!-- PROJECT MODAL -->
    <div id="project-modal" class="fixed inset-0 z-[100] hidden bg-slate-950/95 backdrop-blur-md flex justify-center items-center p-6 opacity-0 transition-all duration-300">
        <div class="bg-[#0f172a] border border-sky-500/30 rounded-[3rem] w-full max-w-2xl p-12 shadow-2xl transform scale-95 transition-all duration-300 border-t-[10px] border-t-sky-500">
            <div class="flex justify-between items-start mb-10">
                <div><span id="proj-sizing" class="px-3 py-1 bg-sky-500 text-slate-950 text-[10px] font-black rounded-lg uppercase mb-4 inline-block tracking-widest"></span><h3 id="proj-name" class="text-3xl font-black text-white tracking-tight leading-tight"></h3></div>
                <button onclick="closeProjectModal()" class="text-slate-500 hover:text-white transition"><i class="fa-solid fa-xmark text-3xl"></i></button>
            </div>
            <div class="space-y-10">
                <div><h4 class="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-3 italic underline">Impacto Estratégico (Valor)</h4><p id="proj-case" class="text-base text-slate-200 leading-relaxed font-light italic bg-slate-950/50 p-6 rounded-2xl"></p></div>
                <div><h4 class="text-[10px] font-black text-sky-400 uppercase tracking-widest mb-3 underline">Objetivo de Ingeniería</h4><p id="proj-objective" class="text-base text-slate-300 leading-relaxed"></p></div>
                <div><h4 class="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Entregables Tangibles (Evidencias)</h4><div id="proj-deliverables" class="flex flex-wrap gap-3"></div></div>
                <div class="flex items-center gap-6 pt-6 border-t border-white/5 font-bold"><div class="flex items-center gap-3 text-sky-400 uppercase tracking-widest text-[10px]"><i class="fa-regular fa-calendar-check"></i><span id="proj-duration"></span></div></div>
            </div>
        </div>
    </div>

    <!-- PROPOSAL MODAL -->
    <div id="proposal-modal" class="fixed inset-0 z-[110] hidden bg-slate-950/95 backdrop-blur-md flex justify-center items-center p-6 opacity-0 transition-all duration-300">
        <div class="bg-[#0f172a] border border-emerald-500/30 rounded-[3rem] w-full max-w-3xl p-12 shadow-2xl transform scale-95 transition-all duration-300 border-t-[10px] border-t-emerald-500 relative">
            <button onclick="closeProposalModal()" class="absolute top-8 right-8 w-12 h-12 rounded-full bg-white/5 hover:bg-white/10 text-slate-500 hover:text-white flex items-center justify-center transition-all"><i class="fa-solid fa-xmark text-xl"></i></button>
            <div class="mb-10 pr-12">
                <span id="prop-tcv" class="px-3 py-1 bg-emerald-500 text-slate-950 text-[10px] font-black rounded-lg uppercase mb-4 inline-block tracking-widest shadow-lg shadow-emerald-500/20"></span>
                <h3 id="prop-name" class="text-4xl font-black text-white tracking-tight leading-tight"></h3>
            </div>
            <div class="space-y-8">
                <div><p id="prop-synthesis" class="text-sm text-slate-300 font-medium leading-relaxed italic bg-slate-900/50 p-6 rounded-2xl border border-white/5"></p></div>
                <div class="grid grid-cols-2 gap-6">
                    <div class="p-6 bg-slate-900/50 rounded-2xl border border-white/5 shadow-inner">
                        <h4 class="text-[10px] font-black text-sky-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2"><i class="fa-solid fa-box-open"></i> Entregables Clave</h4>
                        <ul id="prop-deliverables" class="list-disc pl-4 text-xs text-slate-300 space-y-3 font-medium"></ul>
                    </div>
                    <div class="p-6 bg-emerald-500/5 rounded-2xl border border-emerald-500/10 shadow-inner">
                        <h4 class="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2"><i class="fa-solid fa-chart-line"></i> ROI / Impacto Financiero</h4>
                        <p id="prop-roi" class="text-xs text-emerald-100/90 leading-relaxed font-medium"></p>
                    </div>
                </div>
                <div class="p-6 bg-amber-500/5 rounded-2xl border border-amber-500/10 flex items-start gap-4">
                    <i class="fa-solid fa-triangle-exclamation text-amber-500 mt-1"></i>
                    <div>
                        <h4 class="text-[10px] font-black text-amber-400 uppercase tracking-[0.2em] mb-1">Prerrequisitos & Dependencias</h4>
                        <p id="prop-prereq" class="text-xs text-amber-100/80 leading-relaxed"></p>
                    </div>
                </div>
                <div class="mt-8 pt-8 border-t border-white/10 flex justify-between items-center">
                    <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest">Estado: <span class="text-amber-500">Pendiente Aprobación</span></span>
                    <button class="px-8 py-4 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs uppercase tracking-[0.2em] rounded-xl transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_30px_rgba(16,185,129,0.5)] transform hover:-translate-y-1">Autorizar Iniciativa <i class="fa-solid fa-signature ml-2"></i></button>
                </div>
            </div>
        </div>
    </div>

    <script id="nexus-data" type="application/json">__JSON_DATA__</script>
    ...
    <script>
        ...
        function renderSankey() {
            try {
                ...
                // Add absolute div for rich HTML tooltip
                d3.select("#sankey-tooltip").remove(); // clean if exists
                ...
                // Generate Links deterministically
                nodes.filter(n => n.type==='PROG').forEach((pn, progIdx) => {
                    ...
                    // Fallback determinista si el JSON no tiene mapeadas las dependencias
                    ...
                    if (impacts.length > 0) {
                        ...
                        // Añadir una segunda conexión cruzada para hacer el gráfico más orgánico (solo si hay suficientes impactos y programas)
                ...
                // Asegurar que ninguna torre se quede huerfana (sin conectar)
                ...
                nodeGroups.on("mouseenter", function(event, d) {
                    ...
                    // Positioning (smart flip if too close to edges)
                    const mouseCoords = d3.pointer(event, container);
                    let leftPos = mouseCoords[0] + 30;
                    let topPos = mouseCoords[1] - 40;
                    if(leftPos + 300 > width) leftPos = mouseCoords[0] - 340; // flip left
                    if(topPos + 150 > height) topPos = height - 160; // flip up
        ...
        function renderRoadmap() {
            ...
            document.getElementById('ui-roadmap').innerHTML = m.map((map, idx) => {
                ...
                return `<div class="space-y-8"><div class="flex items-center gap-6 border-b border-white/5 pb-4"><h3 class="text-xs font-black text-white uppercase tracking-[0.5em] italic">${map.l}</h3></div><div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4">${items.map(i => {
                    ...
                    else if (isHigh || idx === 0) impactScore = 'Alta'; // H1 defaults to at least Alta if not critical
        ...
        // Extraer número de target_maturity string (ej. 'Nivel 4 - Optimizado' -> 4.0)
        ...
        function openTower(id) {
            ...
            // RENDER PROFESSIONAL GANTT WITH TIMELINE (36 MONTHS SCALE)
            const allProjects = t.pillars.flatMap(p => p.projects);
            if (allProjects.length) {
                // Timeline Header (0 to 36 months)
                let ganttHtml = `
            ...
            document.getElementById('modal-pillars-rows').innerHTML = t.pillars.map((p, idx) => {
                ...
                <div class="glass rounded-3xl border border-white/5 overflow-hidden shadow-xl mb-8">
                    ...
                    <div class="p-6 lg:p-8 bg-slate-950/40 grid grid-cols-1 lg:grid-cols-3 gap-8 relative overflow-hidden">
                        ...
                        <!-- Col 3: Action Plan (Projects list) -->
                        <div class="space-y-4 lg:border-l lg:border-white/5 lg:pl-8 relative z-10">
    ...
    html_content = html_content.replace("__CLIENT_ID__", client_id.upper())
    html_content = html_content.replace("__JSON_DATA__", json_data)

    with open(pres_dir / "index.html", "w", encoding="utf-8") as f:
        f.write(html_content)

    print(
        f"✅ Dashboard v5.1 (Strategic & Interactive) generado en: {pres_dir}/index.html"
    )


if __name__ == "__main__":
    generate_web_dashboard((argv if argv is not None else sys.argv)[1] if len(argv if argv is not None else sys.argv) > 1 else "ivirma")
