...
def generate_heatmap(payload_path: str, output_image_path: str):
    ...
    if payload_path_obj.exists():
        ...
        for pilar in bp.get("pillars_analysis", []):
            ...
            for finding in pilar.get("health_check_asis", []):
                #
                tef = min(5, max(1, int(round(finding.get("threat_event_frequency", 3)))))
    ...
    color_medium = "#FCF3CF" # Define the standard color for medium-severity risk cells.
    ...
    ax.set_xticklabels([f"LM {i}" for i in range(1, 6)], fontsize=8.5, fontweight="bold", color="#5D6D7E")
    ax.set_yticklabels([f"TEF {i}" for i in range(1, 6)], fontsize=8.5, fontweight="bold", color="#5D6D7E")
    
    # Assign labels to the X (Loss Magnitude) and Y (Threat Event Frequency) axes.
    ax.set_xlabel("Magnitud de Pérdida (LM) — Severidad de Impacto", fontsize=10, fontweight="bold", color="#2E404D", labelpad=10)
    ax.set_ylabel("Frecuencia de Amenaza (TEF) — Probabilidad", fontsize=10, fontweight="bold", color="#2E404D", labelpad=10)
    ax.set_title("Matriz de Exposición Cuantitativa al Riesgo (FAIR)", fontsize=11, fontweight="bold", color="#0072BC", pad=15)
    
    # Disable all plot spines to create a minimalist, frameless heatmap that emphasizes the data grid.
    for spine in ["top", "right", "left", "bottom"]:
        ax.spines[spine].set_visible(False)
        
    ax.tick_params(axis="both", which="both", length=0) # Disable axis tick marks to de-emphasize the axes and focus user attention on the grid.
    ax.set_aspect("equal") # Set a 1:1 aspect ratio to ensure heatmap cells are rendered as squares, preventing spatial distortion.
