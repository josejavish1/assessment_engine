...
from application.build_case_input import read_text
from domain.maturity_band import (
    ANNEX_MATURITY_BANDS,
    resolve_maturity_band,
)
from domain.schemas.annex_synthesis import (
    AnnexPayload,
    GapRowAnnex,
    InitiativeAnnex,
    RiskItemAnnex,
...
from domain.schemas.common import VersionMetadata
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Uso: python -m application.run_executive_annex_synthesizer <client> <tower>"
        )
        sys.exit(1)
