...
async def synthesize_global_tobe(working_dir: str, industry: str):
    ...
    print("🚀 Iniciando Global TO-BE Executive Synthesizer (SOTA 2026 with Self-Healing)...")
    ...
    for file_path in glob.glob(search_pattern):
        with open(file_path, 'r', encoding='utf-8-sig') as f:
            ...
            for pilar in data.get("pillars_analysis", []):
                ...
                tobe_visions.append(f"- Pilar {pilar.get('pilar_name')}: {tobe.get('vision_3_years', '')} / {tobe.get('vision_5_years', '')}")
    ...
    while round_idx <= max_reflection_rounds:
        ...
        if remediation_critique:
            print("   ⚠️ Crítica del Verificador inyectada en el prompt para auto-corrección.")
            
        feedback_prompt = f"\n\nDIRECTIVAS DE CORRECCIÓN DE LA RONDA ANTERIOR:\n{remediation_critique}" if remediation_critique else ""
        ...
        agent_1 = Agent(name="tobe_phase1", model="gemini-2.5-pro", instruction="Responde estrictamente con el JSON pedido.", output_schema=Phase1Strategy)
        res_1 = await run_agent(AdkApp(agent=agent_1), user_id="synthesizer", message=prompt_1, schema=Phase1Strategy)
        
        print("   ➔ Ejecutando Fase 2: Resiliencia Geográfica y Modernización Aplicacional...")
        prompt_2 = (
        ...
        agent_2 = Agent(name="tobe_phase2", model="gemini-2.5-pro", instruction="Responde estrictamente con el JSON pedido.", output_schema=Phase2ResilienceStrategy)
        res_2 = await run_agent(AdkApp(agent=agent_2), user_id="synthesizer", message=prompt_2, schema=Phase2ResilienceStrategy)
        ...
        agent_3 = Agent(name="tobe_phase3", model="gemini-2.5-pro", instruction="Responde estrictamente con el JSON pedido.", output_schema=Phase3Benefits)
        res_3 = await run_agent(AdkApp(agent=agent_3), user_id="synthesizer", message=prompt_3, schema=Phase3Benefits)
        
        print("   ➔ Ejecutando Fase 4: Supuestos Estructurales, Riesgos de Implantación y Siguientes Pasos...")
        prompt_4 = (
        ...
        agent_4 = Agent(name="tobe_phase4", model="gemini-2.5-pro", instruction="Responde estrictamente con el JSON pedido.", output_schema=Phase4RisksAndAssumptions)
        res_4 = await run_agent(AdkApp(agent=agent_4), user_id="synthesizer", message=prompt_4, schema=Phase4RisksAndAssumptions)
        ...
        agent_5 = Agent(name="tobe_phase5", model="gemini-2.5-pro", instruction="Responde estrictamente con el JSON pedido.", output_schema=HighLevelRoadmap)
        res_5 = await run_agent(AdkApp(agent=agent_5), user_id="synthesizer", message=prompt_5, schema=HighLevelRoadmap)

        #
        final_summary = GlobalToBeSummary(
            executive_vision=res_1.get("executive_vision", "") if res_1 else "",
            transformation_strategy=res_1.get("transformation_strategy", "") if res_1 else "",
            ...
            active_active_transition=res_2.get("active_active_transition", "") if res_2 else "",
            app_modernization_relation=res_2.get("app_modernization_relation", "") if res_2 else "",
            strategic_levers=res_3.get("strategic_levers", []) if res_3 else [],
            global_benefits=res_3.get("global_benefits", []) if res_3 else [],
            public_mission_impact=res_3.get("public_mission_impact", "") if res_3 else "",
            assumptions_structural=res_4.get("assumptions_structural", []) if res_4 else [],
            assumptions_technological=res_4.get("assumptions_technological", []) if res_4 else [],
            assumptions_regulatory=res_4.get("assumptions_regulatory", []) if res_4 else [],
            risks_if_not_acted=res_4.get("risks_if_not_acted", []) if res_4 else [],
            risks_of_implementation=res_4.get("risks_of_implementation", []) if res_4 else [],
            next_steps=res_4.get("next_steps", []) if res_4 else [],
            roadmap=res_5 if res_5 else HighLevelRoadmap(phase_1_short_term=[], phase_2_mid_term=[], phase_3_long_term=[])
        )
        ...
        verifier_agent = Agent(name="tobe_verifier", model="gemini-2.5-pro", instruction="Responde estrictamente con el JSON pedido.", output_schema=SynthesisAudit)
        audit_res = await run_agent(AdkApp(agent=verifier_agent), user_id="synthesizer", message=audit_prompt, schema=SynthesisAudit)
        
        if audit_res and audit_res.get("is_approved"):
            print("   ✅ ¡El Verificador de Calidad ha APROBADO la síntesis de forma definitiva!")
            break
        else:
            print("   ❌ El Verificador de Calidad ha RECHAZADO el borrador actual.")
            remediation_critique = audit_res.get("remediation_critique", "Revisa alineación general, extensión de texto y ortografía.")
            print(f"   ├─ Hallucinaciones/Errores: {audit_res.get('hallucinations_detected', [])}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Defines the root schema for the Global TO-BE Executive Synthesizer, a State-of-the-Art (SOTA) 2026 architecture.")
    parser.add_argument("working_dir", type=str, help="Directorio de trabajo")
    parser.add_argument("--industry", type=str, default="critical_infrastructure", help="Perfil sectorial del cliente")
