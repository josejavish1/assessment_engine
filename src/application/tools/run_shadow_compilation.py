...
def run_shadow_compilation() -> None:
    ...
    parser.add_argument("--blueprint-payload", required=True, help="Path to the main blueprint_payload.json")
    parser.add_argument("--approved-annex", required=True, help="Path to the approved_annex.template_payload.json")
    parser.add_argument("--output", required=True, help="Target path for the compiled shadow DOCX document")
    ...
    print(f"   ├─ Conversión a AST completada en memoria en {conversion_time*1000:.2f} ms")
    ...
    minimal_nodes = [n for n in ast.nodes if getattr(n, "type", None) not in {"table", "picture"}]
    minimal_ast = DocumentAST(title=ast.title + " (Minimal)", metadata=ast.metadata, nodes=minimal_nodes)
    ...
    nopix_ast = DocumentAST(title=ast.title + " (No Pictures)", metadata=ast.metadata, nodes=nopix_nodes)
    ...
    notab_ast = DocumentAST(title=ast.title + " (No Tables)", metadata=ast.metadata, nodes=notab_nodes)
    ...
    if len(table_nodes) > 0:
        t0_nodes = [n for n in ast.nodes if getattr(n, "type", None) != "table" or n == table_nodes[0]]
        t0_ast = DocumentAST(title=ast.title + " (Table 0)", metadata=ast.metadata, nodes=t0_nodes)
    ...
    if len(table_nodes) > 1:
        t1_nodes = [n for n in ast.nodes if getattr(n, "type", None) != "table" or n == table_nodes[1]]
        t1_ast = DocumentAST(title=ast.title + " (Table 1)", metadata=ast.metadata, nodes=t1_nodes)
    ...
    if len(table_nodes) > 2:
        t2_nodes = [n for n in ast.nodes if getattr(n, "type", None) != "table" or n == table_nodes[2]]
        t2_ast = DocumentAST(title=ast.title + " (Table 2)", metadata=ast.metadata, nodes=t2_nodes)
    ...
    print(f"   ├─ Compilación física de DOCX completada en {compilation_time*1000:.2f} ms")
    print(f"🎉 ¡Compilación en Sombra V4 Completada con Éxito en {total_time*1000:.2f} ms!")
