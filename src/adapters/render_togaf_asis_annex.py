...
def render_asis_annex(payload_path: str, output_path: str):
    ...
    annex_path = payload_path_obj.parent / f"approved_annex_{tower_id.lower()}.template_payload.json"
    annex_data = {}
    if annex_path.exists():
        with open(annex_path, "r", encoding="utf-8-sig") as af:
            try:
                annex_data = json.load(af)
                print(f"   ├─ Carga exitosa de Síntesis del Anexo Ejecutivo para {tower_id}.")
    ...
    run_title = p_title.add_run(f"Informe de Situación Actual (AS-IS)\nAnexo Técnico: {tower_name}")
    ...
    project_name = vocab.get("project", "Consultoría de Diagnóstico de Infraestructura Crítica y Resiliencia")
    ...
    run_meta = p_meta.add_run(f"{company_name} | Clasificación: {classification}\nVersión {version} | {date_str}")
    ...
    #
    #
    add_heading(doc, vocab.get("control_documental_title", "Control Documental e Histórico de Cambios"), level=1, primary_color_rgb=p_color_rgb)
    hist_table = doc.add_table(rows=3, cols=4)
    hist_table.style = 'Table Grid'
    autofit_table_to_contents(hist_table)
    
    headers_hist = ["Versión", "Fecha", "Comentarios / Cambios Realizados", "Páginas Afectadas"]
    for i, h_txt in enumerate(headers_hist):
        set_cell_text_custom(hist_table.rows[0].cells[i], h_txt, bold=True, font_size=9, color_rgb=RGBColor(255,255,255), align=WD_ALIGN_PARAGRAPH.CENTER)
        shade_cell(hist_table.rows[0].cells[i], p_color_hex)
        
    set_cell_text_custom(hist_table.rows[1].cells[0], "1.0", font_size=8, align=WD_ALIGN_PARAGRAPH.CENTER)
    set_cell_text_custom(hist_table.rows[1].cells[1], "10/05/2026", font_size=8, align=WD_ALIGN_PARAGRAPH.CENTER)
    set_cell_text_custom(hist_table.rows[1].cells[2], "Borrador de diagnóstico técnico inicial.", font_size=8)
    set_cell_text_custom(hist_table.rows[1].cells[3], "Todas", font_size=8, align=WD_ALIGN_PARAGRAPH.CENTER)
    
    set_cell_text_custom(hist_table.rows[2].cells[0], "1.2", font_size=8, align=WD_ALIGN_PARAGRAPH.CENTER)
    set_cell_text_custom(hist_table.rows[2].cells[1], "15/06/2026", font_size=8, align=WD_ALIGN_PARAGRAPH.CENTER)
    set_cell_text_custom(hist_table.rows[2].cells[2], "Refinado cuantitativo de riesgos FAIR e inyección SOTA.", font_size=8)
    set_cell_text_custom(hist_table.rows[2].cells[3], "11-28", font_size=8, align=WD_ALIGN_PARAGRAPH.CENTER)
    
    add_spacer(doc, 24)
    
    add_heading(doc, vocab.get("appendix_a_title", "Índice de Contenidos"), level=1, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, "Nota: Para actualizar el índice en Word, haga clic derecho sobre el texto inferior y seleccione 'Actualizar campos'.", italic=True, text_color_rgb=text_color_rgb)
    add_toc(doc)
    doc.add_page_break()
    ...
    #
    #
    add_heading(doc, vocab.get("intro_metodologia_title", "Introducción y Metodología de Diagnóstico"), level=1, primary_color_rgb=p_color_rgb)
    
    add_heading(doc, vocab.get("objetivo_alcance_title", "Objetivo y Alcance"), level=2, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, f"Este documento técnico anexo detalla de manera exhaustiva el diagnóstico de situación actual (AS-IS) de la torre **{tower_name}** para {client_name}. El objetivo principal es identificar y registrar de manera estructurada las brechas operativas, riesgos de continuidad y obsolescencias tecnológicas dentro del perímetro de evaluación.", text_color_rgb=text_color_rgb)
    add_body_paragraph(doc, "El alcance técnico incluye el inventario de infraestructura y la topología operativa, restringida estrictamente a los sistemas activos de producción del cliente, evaluando el nivel de madurez operativa basándose en evidencias de auditoría recopiladas empíricamente.", text_color_rgb=text_color_rgb)
    
    # The executive summary content is sourced from the `summary_body` field of the synthesis annex payload.
    if exec_sum:
        add_heading(doc, vocab.get("resumen_ejecutivo_title", "Resumen Ejecutivo de la Torre"), level=2, primary_color_rgb=p_color_rgb)
        headline_text = exec_sum.get("headline", "Diagnóstico General")
        add_body_paragraph(doc, f"**{headline_text}**", bold=True, text_color_rgb=text_color_rgb)
        
        summary_body = exec_sum.get("summary_body", "")
        if summary_body:
            add_body_paragraph(doc, summary_body, text_color_rgb=text_color_rgb)
            
        add_heading(doc, vocab.get("impactos_negocio_title", "Principales Impactos de Negocio"), level=3, primary_color_rgb=p_color_rgb)
        ...
        if isinstance(impacts, str):
            ...
            for item in impacts:
                add_body_paragraph(doc, item, style='List Bullet', text_color_rgb=text_color_rgb)

    add_heading(doc, vocab.get("metodologia_madurez_title", "Metodología de Valoración de Madurez"), level=2, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, "La madurez operativa de la torre se calcula mediante un modelo de bloques ponderados alineado con mejores prácticas del sector. La madurez se califica en una escala analítica del 1,00 al 5,00, donde cada nivel determina un estadio de control:")
    add_body_paragraph(doc, "**Nivel 1 - Inicial (0-1.8):** Prácticas inexistentes, inestables o dependientes del esfuerzo heroico de personas clave.", style='List Bullet')
    add_body_paragraph(doc, "**Nivel 2 - Básico (1.8-2.6):** Prácticas existentes de manera parcial o irregular, sin consistencia organizativa.", style='List Bullet')
    add_body_paragraph(doc, "**Nivel 3 - Estandarizado (2.6-3.4):** Procesos formalizados e implantados de manera coherente en toda la organización.", style='List Bullet')
    add_body_paragraph(doc, "**Nivel 4 - Optimizado (3.4-4.2):** Capacidades industrializadas, gobernadas predictivamente y sustentadas en métricas.", style='List Bullet')
    add_body_paragraph(doc, "**Nivel 5 - Avanzado (4.2-5.0):** Procesos dinámicos impulsados por automatización inteligente y mejora continua.", style='List Bullet')

    add_heading(doc, vocab.get("metodologia_fair_title", "Metodología Cuantitativa de Riesgos (FAIR)"), level=2, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, "Para garantizar el rigor ejecutivo y evitar estimaciones cualitativas de riesgo arbitrarias, este diagnóstico utiliza el estándar internacional **FAIR (Factor Analysis of Information Risk)** para modelar la exposición al riesgo en términos financieros reales:", text_color_rgb=text_color_rgb)
    add_body_paragraph(doc, "1. **Frecuencia de Ocurrencia (TEF):** Representa el número estimado de veces al año que un evento de amenaza es capaz de materializar una vulnerabilidad. Se representa cuantitativamente o sobre una escala relativa del 1 al 5.", text_color_rgb=text_color_rgb)
    add_body_paragraph(doc, "2. **Magnitud de la Pérdida (LM):** Representa el impacto económico total promedio estimado por cada evento de pérdida, considerando la interrupción de operaciones, horas de ingeniería de recuperación y penalizaciones regulatorias.", text_color_rgb=text_color_rgb)
    add_body_paragraph(doc, "3. **Exposición de Pérdida Anualizada (ALE):** Representa el riesgo económico real proyectado de manera anualizada, calculado mediante el producto matemático de los factores anteriores:", text_color_rgb=text_color_rgb)
    ...
    add_spacer(doc, 24)
    add_heading(doc, vocab.get("disclaimer_heading_title", "Cláusula de Limitación de Responsabilidad"), level=2, primary_color_rgb=p_color_rgb)
    ...
    #
    #
    add_heading(doc, vocab.get("descripcion_plataforma_title", "Descripción de la Plataforma y Topología Unificada"), level=1, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, "A continuación, se define de manera consolidada y unificada el inventario técnico, arquitectura y estado operativo general del entorno. Esta descripción unifica el contexto tecnológico general para evitar repeticiones innecesarias entre dominios:", text_color_rgb=text_color_rgb)
    
    # Aggregates and deduplicates pillar-specific descriptions into a single text block.
    if pillars:
        main_desc = pillars[0].get("asis_architecture_description", "Descripción técnica de plataforma no disponible.")
        add_body_paragraph(doc, main_desc, text_color_rgb=text_color_rgb)
    else:
        add_body_paragraph(doc, "No hay descripciones topológicas detalladas para esta torre.", text_color_rgb=text_color_rgb)

    # Embeds the generated Tower Maturity Radar chart image into the document.
    if radar_path.exists():
        add_spacer(doc, 20)
        add_heading(doc, vocab.get("radar_chart_title", "Perfil de Madurez General de la Torre (Radar)"), level=2, primary_color_rgb=p_color_rgb)
    ...
    #
    #
    add_heading(doc, vocab.get("matriz_fair_title", "Registro Forense de Vulnerabilidades y Riesgos (Matriz FAIR)"), level=1, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, "Este capítulo constituye el registro bruto y la matriz detallada de riesgos de auditoría. Cada hallazgo está estructurado quirúrgicamente con sus evidencias de diagnóstico y su impacto de negocio:", text_color_rgb=text_color_rgb)
    
    for p_idx, pilar in enumerate(pillars):
        add_heading(doc, f"Dominio: {pilar.get('pilar_name', 'Desconocido')}", level=2, primary_color_rgb=p_color_rgb)
        
        risks = pilar.get("health_check_asis", [])
        if not risks:
            add_body_paragraph(doc, "_No se han registrado hallazgos ni vulnerabilidades específicas para este dominio arquitéctónico en el RAG._", italic=True, text_color_rgb=text_color_rgb)
            doc.add_paragraph()
            continue
        ...
        headers_table = ["ID", "Descripción del Riesgo y Evidencias de Auditoría (Deep Dive)", "Exposición FAIR (ALE)"]
        for i, h_txt in enumerate(headers_table):
            set_cell_text_custom(table.rows[0].cells[i], h_txt, bold=True, font_size=9, color_rgb=RGBColor(255,255,255), align=WD_ALIGN_PARAGRAPH.CENTER)
        ...
        for r_idx, hc in enumerate(risks):
            ...
            set_cell_text_custom(row.cells[0], f"{risk_prefix}{r_idx+1:02d}", bold=True, font_size=9, align=WD_ALIGN_PARAGRAPH.CENTER)
            ...
            p_desc.add_run(clean_text_for_word(hc.get("finding", hc.get("risk_observed", "No descripto."))) + "\n\n")
            
            run_imp = p_desc.add_run("Impacto en Negocio / Operaciones:\n")
            run_imp.bold = True
            p_desc.add_run(clean_text_for_word(hc.get("business_risk", hc.get("impact", "No descripto."))) + "\n\n")
            ...
            if ale and ale > 0:
                ...
                set_cell_text_custom(row.cells[2], calc_txt, font_size=8.5, bold=True, align=WD_ALIGN_PARAGRAPH.RIGHT)
                ...
                set_cell_text_custom(row.cells[2], "N/A", font_size=8.5, align=WD_ALIGN_PARAGRAPH.CENTER)
    ...
    #
    #
    add_heading(doc, vocab.get("evaluacion_madurez_title", "Evaluación de Niveles de Madurez"), level=1, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, "La madurez técnica de la torre de infraestructura se detalla a continuación por cada pilar. El promedio ponderado consolida la brecha que orientará el diseño estratégico de transformación:", text_color_rgb=text_color_rgb)
    ...
    headers_mat = ["Pilar / Capacidad Evaluada", "Score AS-IS", "Meta TO-BE", "Análisis de Brecha y Justificación de Nota"]
    ...
    for r_idx, p in enumerate(pillars):
        ...
        set_cell_text_custom(row.cells[1], f"{p.get('score', 0.0):.2f}", font_size=8.5, align=WD_ALIGN_PARAGRAPH.CENTER)
        set_cell_text_custom(row.cells[2], f"{p.get('target_score', 4.0):.2f}", font_size=8.5, align=WD_ALIGN_PARAGRAPH.CENTER)
    ...
    add_body_paragraph(doc, f"**Puntuación de Madurez AS-IS Obtenida:** {avg_score:.2f} sobre 5,00", bold=True, text_color_rgb=text_color_rgb)
    add_body_paragraph(doc, f"**Puntuación Objetivo TO-BE Proyectada:** {avg_target:.2f} sobre 5,00", bold=True, text_color_rgb=text_color_rgb)
    
    add_body_paragraph(doc, "Esta calificación sitúa a la infraestructura en un estadio operativo fundamental que requiere modernización y automatización urgente para alinearse con los estándares SOTA de continuidad y soberanía. El salto de madurez se sostendrá en la ejecución de las iniciativas recomendadas.", text_color_rgb=text_color_rgb)
    ...
    #
    add_heading(doc, vocab.get("conclusiones_tobe_title", "Conclusiones y Brechas para el TO-BE"), level=1, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, "A partir del análisis técnico y cuantitativo realizado, se consolidan las siguientes conclusiones generales extraídas por nuestro motor de inteligencia para esta torre:", text_color_rgb=text_color_rgb)
    
    add_heading(doc, vocab.get("fortalezas_identificadas_title", "Fortalezas de Infraestructura Identificadas"), level=2, primary_color_rgb=p_color_rgb)
    ...
    if benefits:
        if isinstance(benefits, str):
            ...
            for b in benefits:
                add_body_paragraph(doc, b, style='List Bullet', text_color_rgb=text_color_rgb)
    else:
        add_body_paragraph(doc, "La plataforma cuenta con un núcleo de hardware moderno y soportado de fabricante (Dell, IBM) que constituye una base tecnológica excelente para soportar los modelos de virtualización y datos.", text_color_rgb=text_color_rgb)
    
    add_heading(doc, vocab.get("debilidades_criticas_title", "Debilidades y Desafíos Críticos"), level=2, primary_color_rgb=p_color_rgb)
    ...
    if weaknesses:
        if isinstance(weaknesses, str):
            ...
            for w in weaknesses:
                add_body_paragraph(doc, w, style='List Bullet', text_color_rgb=text_color_rgb)
    else:
        add_body_paragraph(doc, "La excesiva dependencia del conocimiento tácito y de la operación manual secuencial constituye la brecha más crítica del negocio. Ante contingencias graves, los tiempos de recuperación reales se desvían de los RTO teóricos debido a la falta de runbooks automatizados ejecutables.", text_color_rgb=text_color_rgb)
    
    add_heading(doc, vocab.get("next_steps_title", "Siguientes Pasos (Base para el Plan TO-BE)"), level=2, primary_color_rgb=p_color_rgb)
    ...
    if coi:
        ...
        add_body_paragraph(doc, "El diagnóstico cuantitativo AS-IS aquí formulado servirá de red de seguridad y base empírica para el diseño de la arquitectura objetivo TO-BE (Fase II) y la priorización de iniciativas de cambio en el Plan de Actuación TO-DO (Fase III).", text_color_rgb=text_color_rgb)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Uso: python render_togaf_asis_annex.py <blueprint_payload.json> <output_annex.docx>")
        sys.exit(1)
