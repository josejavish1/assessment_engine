...
def render_consolidated_asis(working_dir: str, output_path: str):
    ...
    add_body_paragraph(doc, "A partir del modelo de cuantificación financiera de riesgo **FAIR**, la Expectativa de Pérdida Anualizada (ALE) consolidada agregando todos los riesgos críticos detectados en el assessment de la plataforma asciende a:")
    ...
    run_ale.font.color.rgb = RGBColor(192, 0, 0) # Define the color constant for high-priority alerts or critical status indicators, conforming to standard visual conventions for urgency.
