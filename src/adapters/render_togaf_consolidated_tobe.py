...
def set_cell_text_custom_impl(cell, text, bold, font_size, align, color_rgb):
    # Standard XML safe implementation
    cell.text = ""
    p = cell.paragraphs[0]
    p.alignment = align
...
def render_consolidated_tobe(ast_json_path: str, output_path: str):
    ...
    run_title = p_title.add_run(vocab.get("conclusions_title", "Informe Consolidado de Situación Futura"))
    ...
    # ---------------------------------------------------------
    # 1. RESUMEN EJECUTIVO CON CONTEXTO CORPORATIVO SOTA
    # ---------------------------------------------------------
    add_heading(doc, vocab.get("asis_consolidated_title", "Resumen Ejecutivo"), level=1, primary_color_rgb=p_color_rgb)
    
    add_heading(doc, vocab.get("asis_consolidated_title", "Visión de Estado Objetivo"), level=2, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, global_sum.get("executive_vision", "No definida."))
    
    add_heading(doc, "Estrategia de Transformación de la Plataforma", level=2, primary_color_rgb=p_color_rgb)
    ...
    add_heading(doc, "Pilares Estratégicos de Evolución", level=2, primary_color_rgb=p_color_rgb)
    ...
    # ---------------------------------------------------------
    add_heading(doc, "Propósito, alcance y enfoque del documento", level=1, primary_color_rgb=p_color_rgb)
    ...
    # ---------------------------------------------------------
    add_heading(doc, "Modelo de madurez objetivo", level=1, primary_color_rgb=p_color_rgb)
    
    add_heading(doc, "Principios de evolución de la madurez", level=2, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, "La evolución tecnológica y operativa para alcanzar el nivel objetivo se rige por principios inquebrantables de resiliencia proactiva:")
    add_body_paragraph(doc, "**Resiliencia y Ciber-Resiliencia Primero:** Las inversiones se priorizan para erradicar puntos de fallo únicos y asegurar la continuidad sobre la reducción de costes inmediatos.", style='List Bullet')
    add_body_paragraph(doc, "**Automatización Extrema:** La dependencia del conocimiento táctico y de la operación manual debe eliminarse mediante Runbooks-as-Code.", style='List Bullet')
    add_body_paragraph(doc, "**Soberanía y Evidencia:** Todo salto de madurez debe ser auditable y sustentado por pruebas continuas basadas en evidencias empíricas (Chaos Engineering).", style='List Bullet')

    add_heading(doc, "Estrategia de la resiliencia geográfica", level=2, primary_color_rgb=p_color_rgb)
    
    add_heading(doc, "Estrategia de emplazamiento de CPD", level=3, primary_color_rgb=p_color_rgb)
    ...
    add_heading(doc, "Modelo operativo dual-site avanzado", level=3, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, global_sum.get("active_active_transition", "No definida."))

    add_heading(doc, "Madurez actual vs madurez objetivo", level=2, primary_color_rgb=p_color_rgb)
    
    # Gráfico de radar global con UUID para evitar colisiones concurrentes
    global_labels = [t["tower_name"] for t in towers]
    global_actual = [t["score"] for t in towers]
    global_target = [t["target"] for t in towers]
    if global_labels:
        chart_path = Path(tempfile.gettempdir()) / f"global_radar_tobe_{uuid.uuid4().hex}.png"
    ...
    add_heading(doc, "Matriz Resumen de Evolución por Dominio", level=3, primary_color_rgb=p_color_rgb)
    mat_table = doc.add_table(rows=1, cols=4)
    mat_table.style = 'Table Grid'
    
    headers = ["Dominio Tecnológico", "Situación Actual (AS-IS)", "Objetivo a 3 años", "Visión a 5 años"]
    for i, header in enumerate(headers):
        set_cell_text_custom(mat_table.rows[0].cells[i], header, bold=True, font_size=9, color_rgb=RGBColor(255,255,255), align=WD_ALIGN_PARAGRAPH.CENTER)
    ...
    for r_idx, t in enumerate(towers):
        ...
        set_cell_text_custom(row.cells[0], f"{t['tower_name']}\n({score_text} ➔ {target_text})", bold=True, font_size=8)
        set_cell_text_custom(row.cells[1], "\n".join([f"• {b}" for b in asis_bullets]), font_size=8)
        set_cell_text_custom(row.cells[2], "\n".join([f"• {b}" for b in v3_bullets]), font_size=8)
        set_cell_text_custom(row.cells[3], "\n".join([f"• {b}" for b in v5_bullets]), font_size=8)
    ...
    add_heading(doc, "Relación con la modernización de aplicaciones", level=2, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, global_sum.get("app_modernization_relation", "No definida."))
    ...
    # ---------------------------------------------------------
    add_heading(doc, "Modelo TO-BE por dominio tecnológico", level=1, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, "Este capítulo desglosa en detalle el estado objetivo, las palancas de cambio requeridas y los riesgos de inacción para cada dominio de infraestructura evaluado:")
    
    for t_idx, t in enumerate(towers):
        add_heading(doc, f"Dominio: {t['tower_name']}", level=2, primary_color_rgb=p_color_rgb)
        
        # Gráfico radial individual aislado con UUID
        p_labels = [p["pilar_name"] for p in t["pillars"]]
        p_actual = [p["score"] for p in t["pillars"]]
        p_target = [p["target_score"] for p in t["pillars"]]
        if p_labels:
            chart_path = Path(tempfile.gettempdir()) / f"radar_tobe_{t['tower_name'][:10].replace(' ', '_')}_{uuid.uuid4().hex}.png"
            if generate_radar_chart(p_labels, p_actual, p_target, f"Evolución de Madurez: {t['tower_name']}", str(chart_path)):
        ...
        for p in t["pillars"]:
            ...
            add_heading(doc, "Situación Actual (AS-IS Resumido)", level=4, primary_color_rgb=p_color_rgb)
            add_body_paragraph(doc, p["asis_description"])
            
            add_heading(doc, "Objetivo a 3 años (Nivel 5 Básico)", level=4, primary_color_rgb=p_color_rgb)
            add_body_paragraph(doc, p["vision_3_years"])
            
            add_heading(doc, "Visión Aspiracional a 5 años", level=4, primary_color_rgb=p_color_rgb)
            add_body_paragraph(doc, p["vision_5_years"])
            
            add_heading(doc, "Palancas Principales de Evolución", level=4, primary_color_rgb=p_color_rgb)
            
            # Palancas Estructuradas por categoría con auto-bolding
            doc.add_heading("Palancas Tecnológicas:", level=5)
            tech = p["levers_technology"]
            if not tech: add_body_paragraph(doc, "N/A")
            for item in tech: add_body_paragraph(doc, item, style='List Bullet')
                
            doc.add_heading("Palancas de Procesos:", level=5)
            proc = p["levers_process"]
            if not proc: add_body_paragraph(doc, "N/A")
            for item in proc: add_body_paragraph(doc, item, style='List Bullet')
                
            doc.add_heading("Palancas de Operación:", level=5)
            op = p["levers_operation"]
            if not op: add_body_paragraph(doc, "N/A")
            for item in op: add_body_paragraph(doc, item, style='List Bullet')
            
            add_heading(doc, "Beneficios Esperados en el Nuevo Modelo", level=4, primary_color_rgb=p_color_rgb)
            bens = p["expected_benefits"]
            if not bens: add_body_paragraph(doc, "N/A")
            for item in bens: add_body_paragraph(doc, item, style='List Bullet')
                
            add_heading(doc, "Riesgos si no se actúa y dependencias clave (Cost of Inaction)", level=4, primary_color_rgb=p_color_rgb)
            coi = p["cost_of_inaction_risks"]
            if not coi: add_body_paragraph(doc, "N/A")
            for item in coi: add_body_paragraph(doc, item, style='List Bullet')
    ...
    # ---------------------------------------------------------
    add_heading(doc, "Roadmap de alto nivel y horizontes temporales", level=1, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, "La transformación tecnológica no se plantea como un macroproyecto único, sino como una trayectoria escalonada y secuencial basada en dependencias técnicas lógicas:")
    
    roadmap = global_sum.get("roadmap", {})
    
    add_heading(doc, "Corto plazo (0-12 meses): Estabilización y reducción de riesgos críticos", level=2, primary_color_rgb=p_color_rgb)
    for r in roadmap.get("phase_1_short_term", []):
        add_body_paragraph(doc, r, style='List Bullet')
        
    add_heading(doc, "Medio plazo (Años 2-3): Consolidación del nivel 5 básico", level=2, primary_color_rgb=p_color_rgb)
    for r in roadmap.get("phase_2_mid_term", []):
        add_body_paragraph(doc, r, style='List Bullet')
        
    add_heading(doc, "Largo plazo (Años 4-5): Consolidación del nivel 5 y preparación de la evolución futura", level=2, primary_color_rgb=p_color_rgb)
    ...
    # ---------------------------------------------------------
    add_heading(doc, "Beneficios esperados y posicionamiento futuro", level=1, primary_color_rgb=p_color_rgb)
    
    add_heading(doc, "Beneficios globales y transversales", level=2, primary_color_rgb=p_color_rgb)
    for b in global_sum.get("global_benefits", []):
        add_body_paragraph(doc, b, style='List Bullet')
        
    add_heading(doc, "Impacto sobre la misión estratégica y posicionamiento corporativo", level=2, primary_color_rgb=p_color_rgb)
    ...
    # ---------------------------------------------------------
    add_heading(doc, "Supuestos y riesgos del Modelo TO-BE", level=1, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, "El éxito de la arquitectura objetivo descansa sobre varios supuestos estructurales que requieren una participación activa de la dirección del cliente:")
    
    add_heading(doc, "Supuestos estructurales y organizativos", level=2, primary_color_rgb=p_color_rgb)
    for a in global_sum.get("assumptions_structural", []):
        add_body_paragraph(doc, a, style='List Bullet')
        
    add_heading(doc, "Supuestos tecnológicos y de compatibilidad de arquitectura", level=2, primary_color_rgb=p_color_rgb)
    for a in global_sum.get("assumptions_technological", []):
        add_body_paragraph(doc, a, style='List Bullet')
        
    add_heading(doc, "Supuestos sobre el entorno regulatorio y de cumplimiento", level=2, primary_color_rgb=p_color_rgb)
    for a in global_sum.get("assumptions_regulatory", []):
        add_body_paragraph(doc, a, style='List Bullet')
        
    add_heading(doc, "Riesgos si no se actúa (Cost of Inaction a nivel corporativo)", level=2, primary_color_rgb=p_color_rgb)
    for r in global_sum.get("risks_if_not_acted", []):
        add_body_paragraph(doc, r, style='List Bullet')
        
    add_heading(doc, "Riesgos durante la implantación del plan", level=2, primary_color_rgb=p_color_rgb)
    ...
    # ---------------------------------------------------------
    add_heading(doc, "Conclusiones y Siguientes pasos", level=1, primary_color_rgb=p_color_rgb)
    add_body_paragraph(doc, "El paso hacia el modelo de resiliencia TO-BE es un imperativo estratégico. Se proponen las siguientes acciones inmediatas de gobernanza para iniciar el plan:")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Uso: python render_togaf_consolidated_tobe.py <working_dir/tobe_master_ast.json> <output_doc.docx>")
        sys.exit(1)
