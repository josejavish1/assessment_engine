...
class ParagraphNode(BaseNode):
    ...
    bold: bool = Field(
        False, description="Specifies if the paragraph text is rendered in bold."
    )
    italic: bool = Field(
        False, description="Specifies if the paragraph text is rendered in italics."
    )
    style: str = Field(
        "Normal",
        description="Identifier for the paragraph's style, corresponding to a defined document style (e.g., 'Normal', 'List Bullet').",
    )
    space_after: int = Field(
        6, description="Vertical spacing after the paragraph in points (pt)."
    )
    text_color_rgb: Optional[List[int]] = Field(
        None,
        description="Text color, specified as an RGB tuple of integers, where each value is in the range [0, 255].",
    )
    align: str = Field(
        "JUSTIFY",
        description="Horizontal alignment of the paragraph's content. Valid values are 'LEFT', 'CENTER', 'RIGHT', 'JUSTIFY'.",
    )
    font_size: float = Field(
        10.5, description="Font size of the paragraph in points (pt)."
    )
...
class HeadingNode(BaseNode):
    ...
    level: int = Field(
        ...
        ge=1,
        le=6,
        description="Hierarchical heading level, an integer from 1 (highest) to 6 (lowest).",
    )
    primary_color_rgb: Optional[List[int]] = Field(
        None,
        description="Optional six-digit hexadecimal RGB color string for overriding the default header style.",
    )
...
class CellNode(BaseModel):
    ...
    bold: bool = Field(
        False, description="String literal content to be rendered in a bold style."
    )
    font_size: float = Field(9.0, description="Font size in points (pt).")
    align: str = Field(
        "LEFT",
        description="Horizontal alignment of the paragraph's content. Valid values include LEFT, CENTER, RIGHT, or JUSTIFY.",
    )
    bg_color: Optional[str] = Field(
        None,
        description="Background color of the cell, specified as a six-digit hexadecimal RGB string (e.g., 'FF0000').",
    )
    text_color_rgb: Optional[List[int]] = Field(
        None,
        description="Optional text color override, specified as an RGB tuple of integers, where each value is in the range [0, 255].",
    )
...
class TableRowNode(BaseModel):
    ...
    cells: List[CellNode] = Field(
        ..., description="Ordered sequence of `Cell` nodes that compose the row."
    )
    is_header: bool = Field(
        False, description="Specifies whether the row functions as the table header."
    )
...
class TableNode(BaseNode):
    ...
    headers: Optional[List[str]] = Field(
        None,
        description="Optional list of strings to populate the header row, facilitating simplified table instantiation.",
    )
    rows: List[TableRowNode] = Field(
        default_factory=list,
        description="Ordered sequence of `Row` nodes that compose the table body.",
    )
    style_profile: str = Field(
        "Table Grid",
        description="Identifier for the OpenXML table style to be applied (e.g., 'TableGrid').",
    )
    autofit: bool = Field(
        True,
        description="Enables automatic adjustment of column widths to fit content.",
    )
...
class PictureNode(BaseNode):
    ...
    path: str = Field(
        ...,
        description="Filesystem path (absolute or relative) to the source image file.",
    )
    width_inches: float = Field(
        4.5, description="Display width of the image in inches."
    )
...
DocNode = Union[
    ParagraphNode, HeadingNode, TableNode, SpacerNode, PageBreakNode, PictureNode
]
...
class DocumentAST(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(
        ...
        description="Global document metadata, such as client, date, and versioning information.",
    )
    nodes: List[DocNode] = Field(
        default_factory=list,
        description="Ordered sequence of structural nodes that constitute the primary body of the document.",
    )
