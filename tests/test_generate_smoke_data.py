...
def test_generate_smoke_inputs_writes_vodafone_public_dossier(tmp_path) -> None:
    ...
    assert dossier["profile"]["priority_markets"] == ["Alemania", "Reino Unido", "España", "Italia"]
    ...
    assert any(source["source_type"] == "synthetic" for claim in dossier["claims"] for source in claim["sources"])
