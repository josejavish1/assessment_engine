...
async def test_run_agent_mocked_success(caplog):
    ...
    with patch("assessment_engine.scripts.lib.ai_client._execute_query_with_retry", return_value=mock_response):
        mock_app = MagicMock()
...
async def test_run_agent_empty_response():
    ...
    with patch("assessment_engine.scripts.lib.ai_client._execute_query_with_retry", side_effect=RuntimeError("Respuesta vacía")):
        mock_app = MagicMock()
        mock_agent = MagicMock()
        mock_agent.name = "test_agent"
        mock_agent.model = "gemini-2.5-pro"
