...
def test_sovereign_policy_engine_compilation():
    ...
    p4 = next(p for p in compiled_payload["pillars_analysis"] if p["pilar_id"] == "T2.P4")
    assert len(p4["projects_todo"]) == 1
    assert "Platform Engineering" in p4["projects_todo"][0]["name"]
    
    p1 = next(p for p in compiled_payload["pillars_analysis"] if p["pilar_id"] == "T2.P1")
    ...
    p2 = next(p for p in compiled_payload["pillars_analysis"] if p["pilar_id"] == "T2.P2")
    adoption_plan = next(proj for proj in p2["projects_todo"] if "Adopción de Plataforma" in proj["name"])
    ...
    p5 = next(p for p in compiled_payload["pillars_analysis"] if p["pilar_id"] == "T2.P5")
