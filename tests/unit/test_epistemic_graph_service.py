...
def test_declarative_policy_loading():
    ...
    ci_service = EpistemicGraphService(industry_profile="Energy & Critical Infrastructure")
...
def test_critical_propagation_paths():
    ...
    for i in range(len(paths) - 1):
        assert paths[i]["cumulative_risk_transfer"] >= paths[i+1]["cumulative_risk_transfer"]
...
def test_calculate_risk_centrality():
    ...
    std_devs = {
        "T1": 0.05, "T2": 0.174, "T3": 0.08, "T4": 0.12, "T5": 0.15,
        "T6": 0.06, "T7": 0.04, "T8": 0.09, "T9": 0.05, "T10": 0.11
    }
...
def test_backpropagate_audit_feedback():
    ...
    policy_file = Path(__file__).resolve().parent.parent.parent / "engine_config" / "policies" / "epistemic_dependencies.json"
    pristine_content = policy_file.read_text(encoding="utf-8-sig") if policy_file.exists() else None
