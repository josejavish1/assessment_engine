...
from assessment_engine.infrastructure.ai_client import (
    VertexQueryTimeoutError,
    run_agent,
)
from assessment_engine.infrastructure.json_from_model import parse_json_from_text

# =========================================================================
# ACTION 1: ROBUSTNESS TEST AGAINST VERTEX AI TIMEOUTS
# =========================================================================
...
async def test_run_agent_timeout_error_handling():
    ...
    class MockAppWithTimeout:
        def __init__(self):
            self._agent = type(
                "MockAgent", (object,), {"model": "gemini-2.5-pro", "name": "TestAgent"}
            )()
    ...
    with pytest.raises(VertexQueryTimeoutError) as exc:
        await run_agent(
            mock_app, "user_test_timeout", "This prompt will trigger a timeout"
        )
...
# =========================================================================
# ACTION 3: TRUNCATED STREAMING CHAOS TEST (LLM JSON FUZZING)
# =========================================================================
...
def test_parse_json_from_text_heuristics():
    ...
    assert parse_json_from_text(markdown_fenced) == {
        "executive_vision": "Ensuring edge security"
    }
...
def test_robust_unwrap_and_validate_nested_recovery():
    ...
    try:
        ...
        pytest.fail(
            f"_robust_unwrap_and_validate failed to unwrap nested dictionary: {e}"
        )
