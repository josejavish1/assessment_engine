...
def test_mcp_server_initialization() -> None:
    ...
    assert mcp.name == "Assessment Engine Core", (
        "The FastMCP application name is incorrect."
    )
...
async def test_mcp_server_tools_registration() -> None:
    ...
    for tool_name in expected_tools:
        assert tool_name in registered_names, (
            f"Mandatory MCP tool '{tool_name}' is not registered on the server."
        )
...
async def test_mcp_get_tower_state_robustness() -> None:
    ...
    # 3. --- ARRANGE & ACT ---
    result_content, result_meta = await mcp.call_tool(
        "get_tower_state", {"case_dir": "non_existent_mcp_test_path"}
    )
...
async def test_mcp_abort_and_revert_execution() -> None:
    ...
    with patch("assessment_engine.mcp_server.subprocess.run") as mock_run:
        ...
        mock_run.assert_any_call(
            ["git", "reset", "--hard"], cwd=ANY, check=True, capture_output=True
        )
        mock_run.assert_any_call(
            ["git", "clean", "-fd"], cwd=ANY, check=True, capture_output=True
        )
