...
from assessment_engine.scripts.tools.run_product_owner_orchestrator import generate_plan, load_orchestrator_policy
...
async def run_eval(eval_data: dict[str, Any], policy: dict[str, Any]) -> bool:
    ...
    if "allowed_files_modified" in assertions:
        ...
        if len(allowed) == 0 and len(plan.get("tasks", [])) > 0:
             ...
             print(f"✅ allowed_files_modified: Validado empíricamente.")
    ...
    if "must_mention_golden_path" in assertions and assertions["must_mention_golden_path"]:
        ...
        if "golden_path" not in plan_str and "templates/" not in plan_str:
            print(f"❌ Fallo: must_mention_golden_path no encontrado en el plan.")
            success = False
        else:
            print(f"✅ must_mention_golden_path: Encontrado.")
