...
async def run_eval(eval_data: dict[str, Any], policy: dict[str, Any]) -> bool:
    ...
    if "allowed_files_modified" in assertions:
        ...
        if len(allowed) == 0 and len(plan.get("tasks", [])) > 0:
            print(
                f"❌ Fallo: allowed_files_modified esperaba 0 tareas, se obtuvieron {len(plan.get('tasks', []))}"
            )
            success = False
        else:
    ...
    if (
        "must_mention_golden_path" in assertions
        and assertions["must_mention_golden_path"]
    ):
...
async def main() -> int:
    ...
    )  # Suprimir logs de Vertex para ver los evals
    ...
    if passed < total:
        print(
            "❌ AGENT EVALS FAILED. Red Teaming detectó regresiones de comportamiento."
        )
        return 1
