...
async def test_synthesize_global_tobe_with_self_healing(tmp_path: Path):
            ...
            if verifier_call_count == 1:
                ...
                return {
                    ...
                    "hallucinations_detected": [
                        "Missing critical ENS High security references."
                    ],
    ...
    with patch(
        "assessment_engine.application.run_global_synthesizer_tobe.run_agent",
    ...
    assert verifier_call_count == 2, (
        f"Expected 2 verifier calls (self-healing), but got {verifier_call_count}."
    )
