...
def test_abbreviations_glossary_localization_structure() -> None:
    ...
    for lang in ["es", "en", "pt", "fr", "ja"]:
        ...
        assert isinstance(glossary[lang], dict), f"Glossary for '{lang}' must be a dictionary"
        
        # Verify essential common terms
        for term in ["FAIR", "AS-IS", "CPD", "DR", "ALE"]:
            assert term in glossary[lang], f"Term '{term}' must be in the '{lang}' glossary"
            assert isinstance(glossary[lang][term], str), f"Description of '{term}' in '{lang}' must be a string"
...
def test_dynamic_glossary_regex_boundary_matching() -> None:
    ...
    assert not re.search(r"(?<![a-zA-Z0-9])DR(?![a-zA-Z0-9])", fake_corpus), "DR must not match as a substring of another word"
    
    # "ALE" should not match inside "ALERTA" or "DALE"
    fake_corpus_ale = "Mandaron una ALERTA de tsunami. DALE caña."
    assert not re.search(r"(?<![a-zA-Z0-9])ALE(?![a-zA-Z0-9])", fake_corpus_ale), "ALE must not match as a substring of another word"
...
def test_client_local_glossary_override_logic() -> None:
    ...
    full_glossary = {
        "es": {
            "CPD": "Centro de Proceso de Datos (Sede física)."
        }
    }
    ...
    assert "[Sobrescribir]" in full_glossary["es"]["CPD"], "The local glossary must override the global one"
...
def test_first_use_expansion_quality_gate(capsys) -> None:
    ...
    for term, desc in used_glossary.items():
        ...
        if long_form.lower() not in text_corpus_without_expansion.lower():
            print(f"⚠️  [Glosario] Calidad: La sigla '{term}' se utiliza pero su definición no aparece.")
    ...
    text_corpus_with_expansion = "En el Centro de Proceso de Datos (CPD) se instaló un nuevo climatizador."
    warnings = []
    for term, desc in used_glossary.items():
        long_form = desc.split("(")[0].strip()
    ...
    assert len(warnings) == 0, "No debe haber advertencias si el término expandido está presente"
...
def test_locales_toc_glossary_separation() -> None:
    ...
    for lang in ["es", "en", "pt", "fr", "ja"]:
        ...
        assert "table_of_contents_title" in locales[lang], f"Clave table_of_contents_title ausente en {lang}"
        assert "appendix_a_title" in locales[lang], f"Clave appendix_a_title ausente en {lang}"
        ...
        assert toc != glossary_title, f"Semantic collision detected in {lang}: {toc} == {glossary_title}"
